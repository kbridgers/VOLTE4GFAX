...
int spi_gpio_init(unsigned int cs)
{
   /* enable SSC1 */
	...
	//printf("I am Trying to enable the WDT from the u-boot\n");
	//*(BSP_BIU_WDT_CR) = (1<<31);
//	*BSP_GPIO_P0_ALTSEL0 &= (~0x1000); //GPIO12 used as GPIO 
//        *BSP_GPIO_P0_ALTSEL1 &= (~0x1000); //GPIO12 used as GPIO
//        *BSP_GPIO_P0_DIR |= 0x1000;        //Output direction
//        *BSP_GPIO_P0_OD  |=0x1000;         //Normal mode
//        *BSP_GPIO_P0_PUDSEL |= 0x1000;   //Pull up selected
//        *BSP_GPIO_P0_PUDEN |= 0x1000;    //Pull up enabled
//        *BSP_GPIO_P0_OUT |= 0x1000;        //Output set as HIGH 
//Changed to 0.5
    *BSP_GPIO_P0_ALTSEL0 &= (~0x800); //GPIO12 used as GPIO 
        *BSP_GPIO_P0_ALTSEL1 &= (~0x800); //GPIO12 used as GPIO
        *BSP_GPIO_P0_DIR |= 0x800;        //Output direction
        *BSP_GPIO_P0_OD  |=0x800;         //Normal mode
        *BSP_GPIO_P0_PUDSEL |= 0x800;   //Pull up selected
        *BSP_GPIO_P0_PUDEN |= 0x800;    //Pull up enabled
        *BSP_GPIO_P0_OUT &= (~0x800);        //Output set as HIGH 
    *BSP_GPIO_P0_ALTSEL0 &= (~0x1000); //GPIO12 used as GPIO 
    *BSP_GPIO_P0_ALTSEL1 &= (~0x1000); //GPIO12 used as GPIO
    *BSP_GPIO_P0_DIR |= 0x1000;        //Output direction
    *BSP_GPIO_P0_OD  |=0x1000;         //Normal mode
...
//Pin 2.6 as Telit on/off
//	 mdelay(5000);
    	*BSP_GPIO_P2_ALTSEL0 &= (~0x0040); //GPIO12 used as GPIO 
    	*BSP_GPIO_P2_ALTSEL1 &= (~0x0040); //GPIO12 used as GPIO
    	*BSP_GPIO_P2_DIR |= 0x0040;        //Output direction
    	*BSP_GPIO_P2_OD  |=0x0040;         //Normal mode
    	*BSP_GPIO_P2_PUDSEL |= 0x0040;   //Pull up selected
    	*BSP_GPIO_P2_PUDEN |= 0x0040;    //Pull up enabled
  //  	*BSP_GPIO_P2_OUT |= 0x00040;        //Output set as HIGH    //test for shipped boards
  //  	mdelay(3000);
  //   	*BSP_GPIO_P2_OUT &=(~0x00040);
	return 1;																					 
}
#endif

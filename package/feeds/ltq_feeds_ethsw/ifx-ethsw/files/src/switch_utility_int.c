...
char print_mac_table_str1[]={"MAC_TablePrint"};
char print_device_list_str1[]={"print_device_list"};
 ...
 */
struct switch_command commands1[] =
{
...
unsigned long    cmd_arg1[MAX_COMMAND_ARGS]={};     /* max. 64 command arguments (integer/long/string) */
...
int nr_commands1=sizeof(commands1)/sizeof(commands1[0]);
 ...
 *  NONE
 */
void print_command1( int index )
{
   printf("Usage:  switch_utility %s %s\n", commands1[index].name, commands1[index].param_string);
   printf("\n%s\n", commands1[index].detailed_info);
}
 ...
 */
int check_command1( char *command )
{
   int c;

   for (c = 0; c < nr_commands1; c++)
   {
      if( strcmp(command, commands1[c].name) == 0 )
      {
 ...
 */
int convert_mac_adr_str1( char *mac_adr_str, unsigned char *mac_adr_ptr )
{
 ...
 *   IFX_ERROR on failure
 */
int convert_ip_adr_charStr1( char *ip_adr_str, unsigned int *ip_adr_ptr )
{
...
int convert_ipv4_str1( char *ip_adr_str, unsigned int *ip_adr_ptr )
{
    char *str_ptr=ip_adr_str;
    char *endptr;
    int i, val=0;
...
int convert_ipv6_str1( char *ip_adr_str, void *ip_adr_ptr )
{
 ...
 *   IFX_ERROR on failure
 */
int convert_ip_adr_str1( char *ip_adr_str, unsigned int *ip_adr_ptr )
{
   char *str_ptr=ip_adr_str;
   char *endptr;
   int i, val=0;
 ...
 *   -1 on failure
 */
int convert_parameters1( char **params, int nr_params)
{
   ...
   /*
    * transfer the parameter strings to the cmd_arg1 array by integer conversion
    */
   parg_string = commands1[cmd_index1].param;
   param_pos=0;
   while( (*parg_string) &&
 ...
 *   IFX_SUCCESS
 */
int MeterPort_TABLE_PRINT1( void )
{
...
int ifx_ethsw_vlan_port_member_read1()
{
 ...
 *   IFX_SUCCESS
 */
int print_IPv41(  IFX_uint32_t ipv4 )
{
...
int IGMP_table_entry_print1( union ifx_sw_param *p )
{
   ...
   if (p->multicast_TableEntryRead.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4)
   {
      ...
      if (p->multicast_TableEntryRead.uIP_Gsa.nIPv4 > 0)
         print_IPv41(p->multicast_TableEntryRead.uIP_Gsa.nIPv4);
   }
   else
   {
      /* Todo: IPv6 */
...
int print_igmp_membership_table_entry1( union ifx_sw_param *p )
{
    // Port field
    ...
    {   // IPV6
        print_IPv6_GDA1((IFX_uint16_t*)&p->multicast_TableEntryRead.uIP_Gda.nIPv6);
    }
    ...
    {   // IPV6
        print_IPv6_GSA1((IFX_uint16_t*)&p->multicast_TableEntryRead.uIP_Gsa.nIPv6);
    }
 ...
 *   IFX_SUCCESS
 */
int print_multicast_router_port1( void )
{
   int ret;
   union ifx_sw_param x;
 ...
 *   IFX_SUCCESS
 */
int print_igmp_membership_table1( void )
{
    int ret;
    union ifx_sw_param x;
    ...
    if (x.multicast_TableEntryRead.bLast == IFX_FALSE)
    {
            ...
            printf("-------------------------------------------------------\n");
#if defined(VR9)
              IGMP_table_entry_print1(&x);
#else
              print_igmp_membership_table_entry1(&x);
#endif
            do {
                memset(&x.multicast_TableEntryRead, 0x00, sizeof(x.multicast_TableEntryRead));
 ...
 *   IFX_SUCCESS
 */
int print_mfc_table_entry1( union ifx_sw_param *p )
{
 ...
 *   IFX_SUCCESS
 */
int print_mfc_filter_table1( void )
{
    int ret;
    union ifx_sw_param x;
 ...
 *   IFX_ERROR on failure
 */
int execute_user_command1( const char* cmd )
{
   int retval = IFX_ERROR;

   if( strcmp( print_mac_table_str1, cmd ) == 0)
   {
      if( print_mac_table1() == IFX_SUCCESS)
         retval =  IFX_SUCCESS;
   }
   else if( strcmp( print_device_list_str1, cmd ) == 0)
   {
      if( print_device_list1() == IFX_SUCCESS)
         retval =  IFX_SUCCESS;
   }
   else
      printf("Command %s not found\n", cmd);
 ...
 *   -1 on failure
 */
int build_ioctl_command1( int fd, unsigned int ioctl_command )
{
   ...
   switch(ioctl_command) {
       case IFX_ETHSW_MAC_TABLE_ENTRY_READ:
           ...
           case IFX_ETHSW_VLAN_PORT_MEMBER_READ:
            ifx_ethsw_vlan_port_member_read1();
            break;
       case IFX_ETHSW_MAC_TABLE_ENTRY_ADD:
           memset(&x.MAC_tableAdd, 0x00, sizeof(x.MAC_tableAdd));
           x.MAC_tableAdd.nFId = cmd_arg1[0];
           x.MAC_tableAdd.nPortId = cmd_arg1[1];
           x.MAC_tableAdd.nAgeTimer = cmd_arg1[2];
           x.MAC_tableAdd.bStaticEntry = cmd_arg1[3];
/*           x.MAC_tableAdd.nTrafficClass = cmd_arg1[4];*/
           if( convert_mac_adr_str1((char*)cmd_arg1[4], &x.MAC_tableAdd.nMAC[0]) != IFX_SUCCESS)
               return -1;
       ...
       case IFX_ETHSW_MAC_TABLE_ENTRY_REMOVE:
           ...
           x.MAC_tableRemove.nFId = cmd_arg1[0];
           if( convert_mac_adr_str1((char*)cmd_arg1[1], &x.MAC_tableRemove.nMAC[0]) != IFX_SUCCESS)
               return -1;
       ...
       case IFX_ETHSW_VLAN_PORT_MEMBER_ADD:
           ...
           x.vlan_portMemberAdd.nVId = cmd_arg1[0];
           x.vlan_portMemberAdd.nPortId = cmd_arg1[1];
           x.vlan_portMemberAdd.bVLAN_TagEgress = cmd_arg1[2];
       ...
       case IFX_ETHSW_VLAN_PORT_MEMBER_REMOVE:
           ...
           x.vlan_portMemberRemove.nVId = cmd_arg1[0];
           x.vlan_portMemberRemove.nPortId = cmd_arg1[1];
       ...
       case IFX_ETHSW_VLAN_PORT_CFG_GET:
           ...
           x.vlan_portcfg.nPortId = cmd_arg1[0];
           ioctl_params = (void *)&x.vlan_portcfg;
           break;
       case IFX_ETHSW_VLAN_PORT_CFG_SET:
           memset(&x.vlan_portcfg, 0x00, sizeof(x.vlan_portcfg));
           x.vlan_portcfg.nPortId = cmd_arg1[0];
           x.vlan_portcfg.nPortVId = cmd_arg1[1];
           x.vlan_portcfg.bVLAN_UnknownDrop = cmd_arg1[2];
           x.vlan_portcfg.bVLAN_ReAssign = cmd_arg1[3];
           x.vlan_portcfg.eVLAN_MemberViolation = cmd_arg1[4];
           x.vlan_portcfg.eAdmitMode = cmd_arg1[5];
           x.vlan_portcfg.bTVM = cmd_arg1[6];
           ioctl_params = (void *)&x.vlan_portcfg;
           break;
       case IFX_ETHSW_VLAN_RESERVED_ADD:
           memset(&x.vlan_Reserved, 0x00, sizeof(x.vlan_Reserved));
           x.vlan_Reserved.nVId = cmd_arg1[0];
       ...
       case IFX_ETHSW_MULTICAST_ROUTER_PORT_ADD:
           ...
           x.multicast_RouterPortAdd.nPortId = cmd_arg1[0];
       ...
       case IFX_ETHSW_MULTICAST_ROUTER_PORT_REMOVE:
           ...
           x.multicast_RouterPortRemove.nPortId = cmd_arg1[0];
          ioctl_params = (void *)&x.multicast_RouterPortRemove;
          break;
       case IFX_ETHSW_MULTICAST_ROUTER_PORT_READ:
           memset(&x.multicast_RouterPortRead, 0x00, sizeof(x.multicast_RouterPortRead));
           x.multicast_RouterPortRead.bInitial = cmd_arg1[0];
           x.multicast_RouterPortRead.nPortId = cmd_arg1[1];
       ...
       case IFX_ETHSW_MULTICAST_TABLE_ENTRY_ADD:
            ...
            x.multicast_TableEntryAdd.nPortId = cmd_arg1[0];
            x.multicast_TableEntryAdd.eIPVersion = cmd_arg1[1];
            /* Need to convert the GDA IP addr */
            if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4 )
            {
                if( convert_ipv4_str1((char*)cmd_arg1[2], &x.multicast_TableEntryAdd.uIP_Gda.nIPv4) != IFX_SUCCESS)
                return -1;
            }
            else if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV6 )
            {
                if( convert_ipv6_str1((char*)cmd_arg1[2], &x.multicast_TableEntryAdd.uIP_Gda.nIPv6) != IFX_SUCCESS)
                return -1;
            }
            else
            {
            ...
            if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4 )
            {
                if( convert_ipv4_str1((char*)cmd_arg1[3], &x.multicast_TableEntryAdd.uIP_Gsa.nIPv4) != IFX_SUCCESS)
                  return -1;
            }
            else if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV6 )
            {
                if( convert_ipv6_str1((char*)cmd_arg1[3], &x.multicast_TableEntryAdd.uIP_Gsa.nIPv6) != IFX_SUCCESS)
                  return -1;
            }
            else
            {
            ...
            x.multicast_TableEntryAdd.eModeMember =  cmd_arg1[4];
           ...
           if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4 )
           {
              if( convert_ipv4_str1((char*)cmd_arg1[3], &x.multicast_TableEntryAdd.uIP_Gsa.nIPv4)
                 != IFX_SUCCESS)
                  return -1;
           }
           else if ( x.multicast_TableEntryAdd.eIPVersion == IFX_ETHSW_IP_SELECT_IPV6 )
       ...
       case IFX_ETHSW_MULTICAST_TABLE_ENTRY_REMOVE:
            ...
            x.multicast_TableEntryRemove.nPortId = cmd_arg1[0];
            x.multicast_TableEntryRemove.eIPVersion = cmd_arg1[1];
            /* Need to convert the GDA IP addr */
            if ( x.multicast_TableEntryRemove.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4 )
            {
                if( convert_ipv4_str1((char*)cmd_arg1[2], &x.multicast_TableEntryRemove.uIP_Gda.nIPv4) != IFX_SUCCESS)
                return -1;
            }
            else if ( x.multicast_TableEntryRemove.eIPVersion == IFX_ETHSW_IP_SELECT_IPV6 )
            {
                if( convert_ipv6_str1((char*)cmd_arg1[2], &x.multicast_TableEntryRemove.uIP_Gda.nIPv6) != IFX_SUCCESS)
                return -1;
            }
            else
            {
            ...
            if ( x.multicast_TableEntryRemove.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4 )
            {
                if( convert_ipv4_str1((char*)cmd_arg1[3], &x.multicast_TableEntryRemove.uIP_Gsa.nIPv4) != IFX_SUCCESS)
                return -1;
            }
            else if ( x.multicast_TableEntryRemove.eIPVersion == IFX_ETHSW_IP_SELECT_IPV6 )
            {
                if( convert_ipv6_str1((char*)cmd_arg1[3], &x.multicast_TableEntryRemove.uIP_Gsa.nIPv6) != IFX_SUCCESS)
                return -1;
            }
            else
            {
            ...
            x.multicast_TableEntryRemove.eModeMember = cmd_arg1[4];
  ...
  //            if( convert_ip_adr_str1((char*)cmd_arg1[2], &x.multicast_TableEntryRemove.uIP_Gda.nIPv6[0]) != IFX_SUCCESS)
  //                return -1;
           } else {
            printf ("The input IP Version is not accept.\n");
           }
  ...
  //            if( convert_ip_adr_str1((char*)cmd_arg1[3], &x.multicast_TableEntryRemove.uIP_Gsa.nIPv6[0]) != IFX_SUCCESS)
  //                return -1;
           } else {
            printf ("The input IP Version is not accept.\n");
           }
       ...
       case IFX_ETHSW_MULTICAST_TABLE_ENTRY_READ:
            ...
            print_igmp_membership_table1();
#elif defined(VR9_XX)
           memset(&x.multicast_TableEntryRead, 0x00, sizeof(x.multicast_TableEntryRead));
           x.multicast_TableEntryRead.bInitial = 0;
           x.multicast_TableEntryRead.bLast = 0;
           x.multicast_TableEntryRead.nPortId = cmd_arg1[0];
       ...
       case IFX_ETHSW_MULTICAST_SNOOP_CFG_SET:
           ...
           x.multicast_SnoopCfgSet.eIGMP_Mode = cmd_arg1[0];
           x.multicast_SnoopCfgSet.bIGMPv3 = cmd_arg1[1];
           x.multicast_SnoopCfgSet.bCrossVLAN = cmd_arg1[2];
           x.multicast_SnoopCfgSet.eForwardPort = cmd_arg1[3];
           x.multicast_SnoopCfgSet.nForwardPortId = cmd_arg1[4];
           x.multicast_SnoopCfgSet.nClassOfService = cmd_arg1[5];
           x.multicast_SnoopCfgSet.nRobust = cmd_arg1[6];
           x.multicast_SnoopCfgSet.nQueryInterval = cmd_arg1[7];
           x.multicast_SnoopCfgSet.eSuppressionAggregation = cmd_arg1[8];
           x.multicast_SnoopCfgSet.bFastLeave = cmd_arg1[9];
           x.multicast_SnoopCfgSet.bLearningRouter = cmd_arg1[10];
       ...
       case IFX_ETHSW_STP_BPDU_RULE_SET:
               memset(&x.STP_BPDU_Rule, 0x00, sizeof(x.STP_BPDU_Rule));
               x.STP_BPDU_Rule.eForwardPort = cmd_arg1[0];
               x.STP_BPDU_Rule.nForwardPortId = cmd_arg1[1];
               ioctl_params = (void *)&x.STP_BPDU_Rule;
               break;
#if 0
       case IFX_PSB6970_QOS_PORT_SHAPER_GET:
               memset(&x.qos_portShapterCfg, 0x00, sizeof(x.qos_portShapterCfg));
               x.qos_portShapterCfg.nPort = cmd_arg1[0];
               x.qos_portShapterCfg.nTrafficClass = cmd_arg1[1];
               x.qos_portShapterCfg.eType = cmd_arg1[2];
       ...
       case IFX_PSB6970_QOS_PORT_SHAPER_SET:
               ...
               x.qos_portShapterCfg.nTrafficClass = cmd_arg1[1];
               x.qos_portShapterCfg.eType = cmd_arg1[2];
               x.qos_portShapterCfg.nRate = cmd_arg1[3];
               ioctl_params = (void *)&x.qos_portShapterCfg;
               break;
#else
       ...
       case IFX_PSB6970_QOS_PORT_SHAPER_CFG_SET:
               memset(&x.qos_portShapterCfg, 0x00, sizeof(x.qos_portShapterCfg));
               x.qos_portShapterCfg.nPort = cmd_arg1[0];
               x.qos_portShapterCfg.eWFQ_Type = cmd_arg1[1];
               ioctl_params = (void *)&x.qos_portShapterCfg;
               break;
       case IFX_PSB6970_QOS_PORT_SHAPER_STRICT_GET:
               memset(&x.qos_portShapterStrictCfg, 0x00, sizeof(x.qos_portShapterStrictCfg));
               x.qos_portShapterStrictCfg.nPort = cmd_arg1[0];
               x.qos_portShapterStrictCfg.nTrafficClass = cmd_arg1[1];
       ...
       case IFX_PSB6970_QOS_PORT_SHAPER_STRICT_SET:
               ...
               x.qos_portShapterStrictCfg.nPort = cmd_arg1[0];
               x.qos_portShapterStrictCfg.nTrafficClass = cmd_arg1[1];
               x.qos_portShapterStrictCfg.nRate = cmd_arg1[2];
               ioctl_params = (void *)&x.qos_portShapterStrictCfg;
               break;
       case IFX_PSB6970_QOS_PORT_SHAPER_WFQ_GET:
               memset(&x.qos_portShapterWFQ_Cfg, 0x00, sizeof(x.qos_portShapterWFQ_Cfg));
               x.qos_portShapterWFQ_Cfg.nPort = cmd_arg1[0];
               x.qos_portShapterWFQ_Cfg.nTrafficClass = cmd_arg1[1];
       ...
       case IFX_ETHSW_8021X_EAPOL_RULE_SET:
               memset(&x.PNAC_EAPOL_Rule, 0x00, sizeof(x.PNAC_EAPOL_Rule));
               x.PNAC_EAPOL_Rule.eForwardPort = cmd_arg1[0];
               ioctl_params = (void *)&x.PNAC_EAPOL_Rule;
               break;
       ...
       case IFX_PSB6970_QOS_STORM_SET:
               memset(&x.qos_stormCfg, 0x00, sizeof(x.qos_stormCfg));
               x.qos_stormCfg.bBroadcast = cmd_arg1[0];
               x.qos_stormCfg.bMulticast = cmd_arg1[1];
               x.qos_stormCfg.bUnicast = cmd_arg1[2];
               x.qos_stormCfg.nThreshold10M = cmd_arg1[3];
               x.qos_stormCfg.nThreshold100M = cmd_arg1[4];
               ioctl_params = (void *)&x.qos_stormCfg;
               break;
       case IFX_PSB6970_QOS_MFC_PORT_CFG_GET:
               memset(&x.qos_MfcPortCfg, 0x00, sizeof(x.qos_MfcPortCfg));
               x.qos_MfcPortCfg.nPort = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_MfcPortCfg;
               break;
       case IFX_PSB6970_QOS_MFC_PORT_CFG_SET:
               memset(&x.qos_MfcPortCfg, 0x00, sizeof(x.qos_MfcPortCfg));
               x.qos_MfcPortCfg.nPort = cmd_arg1[0];
               x.qos_MfcPortCfg.bPriorityPort = cmd_arg1[1];
               x.qos_MfcPortCfg.bPriorityEtherType = cmd_arg1[2];
               ioctl_params = (void *)&x.qos_MfcPortCfg;
               break;
       case IFX_PSB6970_QOS_MFC_ADD:
               memset(&x.qos_MfcCfg, 0x00, sizeof(x.qos_MfcCfg));
               x.qos_MfcCfg.sFilterMatchField.eFieldSelection = cmd_arg1[0];
               x.qos_MfcCfg.sFilterMatchField.nPortSrc = cmd_arg1[1];
               x.qos_MfcCfg.sFilterMatchField.nPortDst = cmd_arg1[2];
               x.qos_MfcCfg.sFilterMatchField.nPortSrcRange = cmd_arg1[3];
               x.qos_MfcCfg.sFilterMatchField.nPortDstRange = cmd_arg1[4];
               x.qos_MfcCfg.sFilterMatchField.nProtocol = cmd_arg1[5];
               x.qos_MfcCfg.sFilterMatchField.nEtherType = cmd_arg1[6];
               x.qos_MfcCfg.sFilterInfo.nTrafficClass = cmd_arg1[7];
               x.qos_MfcCfg.sFilterInfo.ePortForward = cmd_arg1[8];
               ioctl_params = (void *)&x.qos_MfcCfg;
               break;
       case IFX_PSB6970_QOS_MFC_DEL:
               memset(&x.qos_MfcMatchField, 0x00, sizeof(x.qos_MfcMatchField));
               x.qos_MfcMatchField.eFieldSelection = cmd_arg1[0];
               x.qos_MfcMatchField.nPortSrc = cmd_arg1[1];
               x.qos_MfcMatchField.nPortDst = cmd_arg1[2];
               x.qos_MfcMatchField.nPortSrcRange = cmd_arg1[3];
               x.qos_MfcMatchField.nPortDstRange = cmd_arg1[4];
               x.qos_MfcMatchField.nProtocol = cmd_arg1[5];
               x.qos_MfcMatchField.nEtherType = cmd_arg1[6];
       ...
       case IFX_ETHSW_CPU_PORT_EXTEND_CFG_SET:
           memset(&x.portextendcfg, 0x00, sizeof(x.portextendcfg));
           x.portextendcfg.eHeaderAdd = cmd_arg1[0];
           x.portextendcfg.bHeaderRemove = cmd_arg1[1];
           if( convert_mac_adr_str1((char*)cmd_arg1[2], &x.portextendcfg.sHeader.nMAC_Src[0]) != IFX_SUCCESS)
               return -1;

           if( convert_mac_adr_str1((char*)cmd_arg1[3], &x.portextendcfg.sHeader.nMAC_Dst[0]) != IFX_SUCCESS)
               return -1;
           x.portextendcfg.sHeader.nEthertype = cmd_arg1[4];
           x.portextendcfg.sHeader.nVLAN_Prio = cmd_arg1[5];
           x.portextendcfg.sHeader.nVLAN_CFI = cmd_arg1[6];
           x.portextendcfg.sHeader.nVLAN_ID = cmd_arg1[7];
           x.portextendcfg.ePauseCtrl = cmd_arg1[8];
           x.portextendcfg.bFcsRemove = cmd_arg1[9];
           x.portextendcfg.nWAN_Ports = cmd_arg1[10];
           ioctl_params = (void *)&x.portextendcfg;
           break;
       case IFX_ETHSW_QOS_DSCP_CLASS_GET:
           memset(&x.qos_dscpclasscfgget, 0x00, sizeof(x.qos_dscpclasscfgget));
       ...
       case IFX_ETHSW_QOS_DSCP_CLASS_SET:
           ...
           if (cmd_arg1[1] < 16) {
             ...
             value = cmd_arg1[0];
             x.qos_dscpclasscfgset.nTrafficClass[value] = cmd_arg1[1];
             ioctl_params = (void *)&x.qos_dscpclasscfgset;
          }
          else
       ...
       case IFX_ETHSW_QOS_PCP_CLASS_SET:
           if (cmd_arg1[1] < 16) {
             ...
             value = cmd_arg1[0];
             x.qos_pcpclasscfgset.nTrafficClass[value] = cmd_arg1[1];
             ioctl_params = (void *)&x.qos_pcpclasscfgset;
           }
           else
            printf("The Traffic class value can't not be over 16\n");
       ...
       case IFX_ETHSW_QOS_PORT_REMARKING_CFG_GET:
               ...
               x.qos_portremarking.nPortId = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_portremarking;
               break;
       case IFX_ETHSW_QOS_PORT_REMARKING_CFG_SET:
               memset(&x.qos_portremarking, 0x00, sizeof(x.qos_portremarking));
               x.qos_portremarking.nPortId = cmd_arg1[0];
               x.qos_portremarking.eDSCP_IngressRemarkingEnable = cmd_arg1[1];
               x.qos_portremarking.bDSCP_EgressRemarkingEnable = cmd_arg1[2];
               x.qos_portremarking.bPCP_IngressRemarkingEnable = cmd_arg1[3];
               x.qos_portremarking.bPCP_EgressRemarkingEnable = cmd_arg1[4];
               ioctl_params = (void *)&x.qos_portremarking;
               break;
       case IFX_ETHSW_QOS_QUEUE_PORT_GET:
               memset(&x.qos_queueport, 0x00, sizeof(x.qos_queueport));
               x.qos_queueport.nPortId = cmd_arg1[0];
               x.qos_queueport.nTrafficClassId = cmd_arg1[1];
               ioctl_params = (void *)&x.qos_queueport;
               break;
       case IFX_ETHSW_QOS_QUEUE_PORT_SET:
               memset(&x.qos_queueport, 0x00, sizeof(x.qos_queueport));
               x.qos_queueport.nQueueId = cmd_arg1[0];
               x.qos_queueport.nPortId = cmd_arg1[1];
               x.qos_queueport.nTrafficClassId = cmd_arg1[2];
               ioctl_params = (void *)&x.qos_queueport;
               break;
       ...
       case IFX_ETHSW_QOS_WRED_CFG_SET:
               memset(&x.qos_wredcfg, 0x00, sizeof(x.qos_wredcfg));
               x.qos_wredcfg.eProfile = cmd_arg1[0];
               x.qos_wredcfg.nRed_Min = cmd_arg1[1];
               x.qos_wredcfg.nRed_Max = cmd_arg1[2];
               x.qos_wredcfg.nYellow_Min = cmd_arg1[3];
               x.qos_wredcfg.nYellow_Max = cmd_arg1[4];
               x.qos_wredcfg.nGreen_Min = cmd_arg1[5];
               x.qos_wredcfg.nGreen_Max = cmd_arg1[6];
               ioctl_params = (void *)&x.qos_wredcfg;
               break;
       case IFX_ETHSW_QOS_WRED_QUEUE_CFG_GET:
               memset(&x.qos_wredqueuecfg, 0x00, sizeof(x.qos_wredqueuecfg));
               x.qos_wredqueuecfg.nQueueId = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_wredqueuecfg;
               break;
       case IFX_ETHSW_QOS_WRED_QUEUE_CFG_SET:
               memset(&x.qos_wredqueuecfg, 0x00, sizeof(x.qos_wredqueuecfg));
               x.qos_wredqueuecfg.nQueueId = cmd_arg1[0];
 /*              x.qos_wredqueuecfg.eProfile = cmd_arg1[1]; */
               x.qos_wredqueuecfg.nRed_Min = cmd_arg1[2];
               x.qos_wredqueuecfg.nRed_Max = cmd_arg1[3];
               x.qos_wredqueuecfg.nYellow_Min = cmd_arg1[4];
               x.qos_wredqueuecfg.nYellow_Max = cmd_arg1[5];
               x.qos_wredqueuecfg.nGreen_Min = cmd_arg1[6];
               x.qos_wredqueuecfg.nGreen_Max = cmd_arg1[7];
               ioctl_params = (void *)&x.qos_wredqueuecfg;
               break;
       case IFX_ETHSW_QOS_SCHEDULER_CFG_GET:
               memset(&x.qos_schedulecfg, 0x00, sizeof(x.qos_schedulecfg));
               x.qos_schedulecfg.nQueueId = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_schedulecfg;
               break;
       case IFX_ETHSW_QOS_SCHEDULER_CFG_SET:
               memset(&x.qos_schedulecfg, 0x00, sizeof(x.qos_schedulecfg));
               x.qos_schedulecfg.nQueueId = cmd_arg1[0];
               x.qos_schedulecfg.eType = cmd_arg1[1];
               x.qos_schedulecfg.nWeight = cmd_arg1[2];
               ioctl_params = (void *)&x.qos_schedulecfg;
               break;
       case IFX_ETHSW_QOS_STORM_CFG_SET:
               memset(&x.qos_stormcfg, 0x00, sizeof(x.qos_stormcfg));
               x.qos_stormcfg.nMeterId = cmd_arg1[0];
               x.qos_stormcfg.bBroadcast = cmd_arg1[1];
               x.qos_stormcfg.bMulticast = cmd_arg1[2];
               x.qos_stormcfg.bUnknownUnicast = cmd_arg1[3];
               ioctl_params = (void *)&x.qos_stormcfg;
               break;
...
*/
       case IFX_ETHSW_QOS_SHAPER_CFG_GET:
               ...
               x.qos_shappercfg.nRateShaperId = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_shappercfg;
               break;
       case IFX_ETHSW_QOS_SHAPER_CFG_SET:
               memset(&x.qos_shappercfg, 0x00, sizeof(x.qos_shappercfg));
               x.qos_shappercfg.nRateShaperId = cmd_arg1[0];
               x.qos_shappercfg.bEnable = cmd_arg1[1];
               x.qos_shappercfg.nCbs = cmd_arg1[2];
               x.qos_shappercfg.nRate = cmd_arg1[3];
               ioctl_params = (void *)&x.qos_shappercfg;
               break;
       case IFX_ETHSW_QOS_SHAPER_QUEUE_ASSIGN:
               memset(&x.qos_shapperqueue, 0x00, sizeof(x.qos_shapperqueue));
               x.qos_shapperqueue.nRateShaperId = cmd_arg1[0];
               x.qos_shapperqueue.nQueueId = cmd_arg1[1];
               ioctl_params = (void *)&x.qos_stormcfg;
               break;
       ...
       case IFX_ETHSW_QOS_METER_CFG_GET:
               ...
               x.qos_metercfg.nMeterId = cmd_arg1[0];
               ioctl_params = (void *)&x.qos_metercfg;
               break;
       case IFX_ETHSW_QOS_METER_CFG_SET:
               memset(&x.qos_metercfg, 0x00, sizeof(x.qos_metercfg));
               x.qos_metercfg.bEnable = cmd_arg1[0];
               x.qos_metercfg.nMeterId = cmd_arg1[1];
               x.qos_metercfg.nCbs = cmd_arg1[2];
               x.qos_metercfg.nEbs = cmd_arg1[3];
               x.qos_metercfg.nRate = cmd_arg1[4];
               ioctl_params = (void *)&x.qos_metercfg;
               break;
       ...
       case IFX_ETHSW_QOS_METER_PORT_ASSIGN:
               ...
               x.qos_meterport.nMeterId = cmd_arg1[0];
               x.qos_meterport.eDir = cmd_arg1[1];
               x.qos_meterport.nPortIngressId = cmd_arg1[2];
               x.qos_meterport.nPortEgressId = cmd_arg1[3];
       ...
       case IFX_FLOW_PCE_RULE_WRITE:
           ...
           /* Rule for pattern */
           x.pce_rule.pattern.nIndex = cmd_arg1[0];
           x.pce_rule.pattern.bEnable = cmd_arg1[1];
           x.pce_rule.pattern.bPortIdEnable = cmd_arg1[2];
           x.pce_rule.pattern.nPortId = cmd_arg1[3];
           x.pce_rule.pattern.bDSCP_Enable = cmd_arg1[4];
           x.pce_rule.pattern.nDSCP = cmd_arg1[5];
           x.pce_rule.pattern.bPCP_Enable = cmd_arg1[6];
           x.pce_rule.pattern.nPCP = cmd_arg1[7];
           x.pce_rule.pattern.bPktLngEnable = cmd_arg1[8];
           x.pce_rule.pattern.nPktLng = cmd_arg1[9];
           x.pce_rule.pattern.nPktLngRange = cmd_arg1[10];

           x.pce_rule.pattern.bMAC_DstEnable = cmd_arg1[11];
           if( convert_mac_adr_str1((char*)cmd_arg1[12], &x.pce_rule.pattern.nMAC_Dst[0]) != IFX_SUCCESS)
               return -1;

           x.pce_rule.pattern.nMAC_DstMask = cmd_arg1[13];

           x.pce_rule.pattern.bMAC_SrcEnable = cmd_arg1[14];
           if( convert_mac_adr_str1((char*)cmd_arg1[15], &x.pce_rule.pattern.nMAC_Src[0]) != IFX_SUCCESS)
               return -1;

           x.pce_rule.pattern.nMAC_SrcMask = cmd_arg1[16];

           x.pce_rule.pattern.bAppDataMSB_Enable = cmd_arg1[17];
           x.pce_rule.pattern.nAppDataMSB = cmd_arg1[18];
           x.pce_rule.pattern.bAppMaskRangeMSB_Select = cmd_arg1[19];
           x.pce_rule.pattern.nAppMaskRangeMSB = cmd_arg1[20];

           x.pce_rule.pattern.bAppDataLSB_Enable = cmd_arg1[21];
           x.pce_rule.pattern.nAppDataLSB = cmd_arg1[22];
           x.pce_rule.pattern.bAppMaskRangeLSB_Select = cmd_arg1[23];
           x.pce_rule.pattern.nAppMaskRangeLSB = cmd_arg1[24];

           x.pce_rule.pattern.eDstIP_Select = cmd_arg1[25];
           if( convert_ip_adr_charStr1((char*)cmd_arg1[26], &x.pce_rule.pattern.nDstIP.nIPv4) != IFX_SUCCESS)
              return -1;

           x.pce_rule.pattern.nDstIP_Mask = cmd_arg1[27];

           x.pce_rule.pattern.eSrcIP_Select = cmd_arg1[28];
           if( convert_ip_adr_charStr1((char*)cmd_arg1[29], &x.pce_rule.pattern.nSrcIP.nIPv4) != IFX_SUCCESS)
              return -1;

           x.pce_rule.pattern.nSrcIP_Mask = cmd_arg1[30];

           x.pce_rule.pattern.bEtherTypeEnable =cmd_arg1[31];
           x.pce_rule.pattern.nEtherType = cmd_arg1[32];
           x.pce_rule.pattern.nEtherTypeMask = cmd_arg1[33];

           x.pce_rule.pattern.bProtocolEnable = cmd_arg1[34];
           x.pce_rule.pattern.nProtocol = cmd_arg1[35];
           x.pce_rule.pattern.nProtocolMask = cmd_arg1[36];

           x.pce_rule.pattern.bSessionIdEnable = cmd_arg1[37];
           x.pce_rule.pattern.nSessionId = cmd_arg1[38];

           x.pce_rule.pattern.bVid = cmd_arg1[39];
           x.pce_rule.pattern.nVid = cmd_arg1[40];

           /* Rule for Action */
           x.pce_rule.action.eTrafficClassAction = cmd_arg1[41];
           x.pce_rule.action.nTrafficClassAlternate = cmd_arg1[42];
           x.pce_rule.action.eSnoopingTypeAction = cmd_arg1[43];
           x.pce_rule.action.eLearningAction = cmd_arg1[44];
           x.pce_rule.action.eIrqAction = cmd_arg1[45];
           x.pce_rule.action.eCrossStateAction = cmd_arg1[46];
           x.pce_rule.action.eCritFrameAction = cmd_arg1[47];
           x.pce_rule.action.eTimestampAction = cmd_arg1[48];
           x.pce_rule.action.ePortMapAction = cmd_arg1[49];
           x.pce_rule.action.nForwardPortMap = cmd_arg1[50];
           x.pce_rule.action.bRemarkAction = cmd_arg1[51];

           x.pce_rule.action.bRemarkPCP = cmd_arg1[52];
           x.pce_rule.action.bRemarkDSCP = cmd_arg1[53];
           x.pce_rule.action.bRemarkClass = cmd_arg1[54];

           x.pce_rule.action.eMeterAction = cmd_arg1[55];
           x.pce_rule.action.nMeterId = cmd_arg1[56];

           x.pce_rule.action.bRMON_Action = cmd_arg1[57];
           x.pce_rule.action.nRMON_Id = cmd_arg1[58];

           x.pce_rule.action.eVLAN_Action = cmd_arg1[59];
           x.pce_rule.action.nVLAN_Id = cmd_arg1[60];
           x.pce_rule.action.eVLAN_CrossAction = cmd_arg1[61];
       ...
       case IFX_ETHSW_CFG_SET:
           memset(&x.cfg_Data, 0x00, sizeof(x.cfg_Data));
           //x.cfg_Data.bEnable = cmd_arg1[0];
           x.cfg_Data.eMAC_TableAgeTimer = cmd_arg1[0];
           x.cfg_Data.bVLAN_Aware = cmd_arg1[1];
           // x.cfg_Data.eMulticastTableAgeTimer = cmd_arg1[3];
           x.cfg_Data.nMaxPacketLen = cmd_arg1[2];
           // x.portcfg.bLearningLimitAction = cmd_arg1[3];
           x.cfg_Data.bPauseMAC_ModeSrc = cmd_arg1[3];
           if( convert_mac_adr_str1((char*)cmd_arg1[4], &x.cfg_Data.nPauseMAC_Src[0]) != IFX_SUCCESS)
               return -1;
           // x.cfg_Data.nPauseMAC_Src[6] = cmd_arg1[5];
          ioctl_params = (void *)&x.cfg_Data;
           break;
       case IFX_ETHSW_PORT_CFG_GET:
           memset(&x.portcfg, 0x00, sizeof(x.portcfg));
           x.portcfg.nPortId = cmd_arg1[0];
          ioctl_params = (void *)&x.portcfg;
           break;
       case IFX_ETHSW_PORT_CFG_SET:
           memset(&x.portcfg, 0x00, sizeof(x.portcfg));
           x.portcfg.nPortId = cmd_arg1[0];
           x.portcfg.eEnable = cmd_arg1[1];
           x.portcfg.bUnicastUnknownDrop = cmd_arg1[2];
           x.portcfg.bMulticastUnknownDrop = cmd_arg1[3];
           x.portcfg.bReservedPacketDrop = cmd_arg1[4];
           x.portcfg.bBroadcastDrop = cmd_arg1[5];
           x.portcfg.bAging = cmd_arg1[6];
           // x.portcfg.bLearningLimitAction = cmd_arg1[7];
           x.portcfg.bLearningMAC_PortLock = cmd_arg1[7];
           x.portcfg.nLearningLimit = cmd_arg1[8]; 
           x.portcfg.ePortMonitor = cmd_arg1[9];
           x.portcfg.eFlowCtrl = cmd_arg1[10];
           ioctl_params = (void *)&x.portcfg;
           break;
       case IFX_ETHSW_PORT_LINK_CFG_GET:
           memset(&x.portlinkcfgGet, 0x00, sizeof(x.portlinkcfgGet));
           x.portlinkcfgGet.nPortId = cmd_arg1[0];
          ioctl_params = (void *)&x.portlinkcfgGet;
           break;
       case IFX_ETHSW_PORT_LINK_CFG_SET:
           memset(&x.portlinkcfgSet, 0x00, sizeof(x.portlinkcfgSet));
           x.portlinkcfgSet.nPortId = cmd_arg1[0];
           x.portlinkcfgSet.bDuplexForce = cmd_arg1[1];
           x.portlinkcfgSet.eDuplex = cmd_arg1[2];
           x.portlinkcfgSet.bSpeedForce = cmd_arg1[3];
           x.portlinkcfgSet.eSpeed = cmd_arg1[4];
           x.portlinkcfgSet.bLinkForce = cmd_arg1[5];
           x.portlinkcfgSet.eLink = cmd_arg1[6];
           x.portlinkcfgSet.eMII_Mode = cmd_arg1[7];
           x.portlinkcfgSet.eMII_Type = cmd_arg1[8];
           x.portlinkcfgSet.eClkMode = cmd_arg1[9];
  ...
  // printf("retval if ioctl [%d] is [%d]\n", ioctl_command, retval);
   ...
   switch(ioctl_command) {
              ...
              if (x.MAC_tableRead.bLast == 0)
              {
                ...
                printf("  %02x:%02x:%02x:%02x:%02x:%02x  |  %4d    | %d   | %d\n",
                 ...
                 do {
                   memset(&x.MAC_tableRead, 0x00, sizeof(x.MAC_tableRead));
                   x.MAC_tableRead.bInitial = 0;
                   x.MAC_tableRead.nPortId = cmd_arg1[1];
...
#ifdef VERBOSE_PRINT
#if defined(VR9)
            print_multicast_router_port1();
#else
            printf("\tMULTICAST PortId           = %x\n", x.multicast_RouterPortRead.nPortId);
#endif
#else
            printf("nPortId=0x%x\n", x.multicast_RouterPortRead.nPortId);
#endif
         ...
         case IFX_ETHSW_MULTICAST_SNOOP_CFG_GET:
             ...
             printf("\tMULTICAST SNOOP QueryInterval (HEX) = %x\n", x.multicast_SnoopCfgGet.nQueryInterval);
             ...
             printf("nQueryInterval=%x ", x.multicast_SnoopCfgGet.nQueryInterval);
           ...
           case IFX_ETHSW_QOS_PORT_REMARKING_CFG_GET:
                ...
                printf("\teDSCP_IngressRemarkingEnable = %x\n", x.qos_portremarking.eDSCP_IngressRemarkingEnable);
                printf("\tbDSCP_EgressRemarkingEnable  = %x\n", x.qos_portremarking.bDSCP_EgressRemarkingEnable);
                printf("\tbPCP_IngressRemarkingEnable  = %x\n", x.qos_portremarking.bPCP_IngressRemarkingEnable);
                printf("\tbPCP_EgressRemarkingEnable   = %x\n", x.qos_portremarking.bPCP_EgressRemarkingEnable);
#else
                printf("\tnPortId=%x ", x.qos_portremarking.nPortId);
                printf("\teDSCP_IngressRemarkingEnable=%x ", x.qos_portremarking.eDSCP_IngressRemarkingEnable);
                printf("\tbDSCP_EgressRemarkingEnable=%x ", x.qos_portremarking.bDSCP_EgressRemarkingEnable);
                printf("\tbPCP_IngressRemarkingEnable=%x ", x.qos_portremarking.bPCP_IngressRemarkingEnable);
                printf("\tbPCP_EgressRemarkingEnable=%x\n", x.qos_portremarking.bPCP_EgressRemarkingEnable);
#endif
           break;
         case IFX_ETHSW_CFG_GET:
...
/*                printf("\tMax Packet Len      = %d\n", x.cfg_Data.bLearningLimitAction); */
       ...
       case IFX_ETHSW_PORT_RGMII_CLK_CFG_GET:
        ...
        break;
         ...
         case IFX_ETHSW_QOS_METER_PORT_GET:
#ifdef VERBOSE_PRINT
               MeterPort_TABLE_PRINT1();
#else
                printf("nMeterId = %x ", x.qos_meterport.nMeterId);
                printf("eDir= %x ", x.qos_meterport.eDir);
                printf("nPortIngressId= %x ", x.qos_meterport.nPortIngressId);
                printf("nPortEgressId= %x\n", x.qos_meterport.nPortEgressId);
#endif
             break;
         case IFX_ETHSW_QOS_WRED_QUEUE_CFG_GET:
#ifdef VERBOSE_PRINT
                printf("QoS queue index = %x\n", x.qos_wredqueuecfg.nQueueId);
 //               printf("Drop Probability Profile = %x\n", x.qos_wredqueuecfg.eProfile);
                printf("WRED Red Threshold Min = %x\n", x.qos_wredqueuecfg.nRed_Min);
                printf("WRED Red Threshold Max = %x\n", x.qos_wredqueuecfg.nRed_Max);
                printf("WRED Yellow Threshold Min = %x\n", x.qos_wredqueuecfg.nYellow_Min);
                printf("WRED Yellow Threshold Max = %x\n", x.qos_wredqueuecfg.nYellow_Max);
                printf("WRED Green Threshold Min = %x\n", x.qos_wredqueuecfg.nGreen_Min);
                printf("WRED Green Threshold Max = %x\n", x.qos_wredqueuecfg.nGreen_Max);
#else
                printf("nQueueId= %x ", x.qos_wredqueuecfg.nQueueId);
 //               printf("eProfile= %x ", x.qos_wredqueuecfg.eProfile);
                printf("nRed_Min= %x ", x.qos_wredqueuecfg.nRed_Min);
                printf("nRed_Max= %x ", x.qos_wredqueuecfg.nRed_Max);
                printf("nYellow_Min= %x ", x.qos_wredqueuecfg.nYellow_Min);
                printf("nYellow_Max= %x ", x.qos_wredqueuecfg.nYellow_Max);
                printf("nGreen_Min= %x ", x.qos_wredqueuecfg.nGreen_Min);
                printf("nGreen_Max= %x\n", x.qos_wredqueuecfg.nGreen_Max);
#endif
             break;
         case IFX_ETHSW_QOS_SCHEDULER_CFG_GET:
                ...
                printf("\tScheduler Type  = %x\n", x.qos_schedulecfg.eType);
                printf("\tRatio = 0x%x\n", x.qos_schedulecfg.nWeight);
#else
                printf("nQueueId=%x ",x.qos_schedulecfg.nQueueId);
                printf("eType=%x\n", x.qos_schedulecfg.eType);
                printf("Ratio= %x\n", x.qos_schedulecfg.nWeight);
#endif
             break;
         case IFX_ETHSW_QOS_SHAPER_CFG_GET:
#ifdef VERBOSE_PRINT
                printf("\tRate shaper index = %x\n", x.qos_shappercfg.nRateShaperId);
                printf("\tEnable or Disable the rate shaperx = %x\n", x.qos_shappercfg.bEnable);
                printf("\tCommitted Burst Size = %d\n", x.qos_shappercfg.nCbs);
                printf("\tRate [kbit/s]= %d\n", x.qos_shappercfg.nRate);
#else
                printf("nRateShaperId=%x\n", x.qos_shappercfg.nRateShaperId);
                printf("bEnable=%x ", x.qos_shappercfg.bEnable);
         ...
         case IFX_ETHSW_QOS_STORM_CFG_GET:
#ifdef VERBOSE_PRINT
                printf("\tMeter index = %x\n", x.qos_stormcfg.nMeterId);
                printf("\tbroadcast traffic= %x\n", x.qos_stormcfg.bBroadcast);
                printf("\tmulticast traffic = %x\n", x.qos_stormcfg.bMulticast);
                printf("\tunknown unicast traffic= %x\n", x.qos_stormcfg.bUnknownUnicast);
#else
       ...
       case IFX_ETHSW_CPU_PORT_EXTEND_CFG_GET:
         ...
         case IFX_ETHSW_QOS_WRED_CFG_GET:
                ...
                printf("\tnRed_Min=%x ", x.qos_wredcfg.nRed_Min);
                printf("\tnRed_Max=%x ", x.qos_wredcfg.nRed_Max);
                printf("\tnYellow_Min=%x ", x.qos_wredcfg.nYellow_Min);
                printf("\tnYellow_Max=%x ", x.qos_wredcfg.nYellow_Max);
                printf("\tnGreen_Min=%x ", x.qos_wredcfg.nGreen_Min);
                printf("\tnGreen_Max=%x\n", x.qos_wredcfg.nGreen_Max);
#endif
            break;
         case IFX_FLOW_PCE_RULE_READ:
                ...
                printf("\tDestination MAC address mask         = %x\n", x.pce_rule.pattern.nMAC_DstMask);
                ...
                printf("\tSource MAC address mask              = %x\n", x.pce_rule.pattern.nMAC_SrcMask);
...
int main (int argc, char **argv)
{
   ...
   if( (cmd_index1=check_command1( argv[1] )) == -1 )
   {
    ...
    */
   if( strlen(commands1[cmd_index1].param) != (argc-2) )
   {
      printf("Invalid number of parameters: is %d, should be %d\n",
         (argc-2), strlen(commands1[cmd_index1].param));
      print_command1( cmd_index1 );
      close(switch_fd);
      return (IFX_ERROR);
   }

   /* convert parameter strings included in argv[2]... to cmd_arg1 array */
   if( convert_parameters1( &argv[2], argc-2) == -1 )
   {
   ...
   if(commands1[cmd_index1].ioctl_command != 0xFFFFFFFF)
      retval = build_ioctl_command1( switch_fd, commands1[cmd_index1].ioctl_command );
   else
      retval = execute_user_command1( commands1[cmd_index1].name );

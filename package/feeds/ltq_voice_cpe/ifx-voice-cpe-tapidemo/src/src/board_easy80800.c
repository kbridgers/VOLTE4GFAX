...
/** Name of board */
static const char* BOARD_NAME_EASY80800 = "EASY80800";
...
/* static IFX_char_t* GPIO_PORT_NAME = "/dev/danube-port";*/
...
/* ============================= */
IFX_return_t BOARD_Easy80800_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath);
IFX_return_t BOARD_Easy80800_PrepareNames(BOARD_t* pBoard);
IFX_return_t BOARD_Easy80800_CfgPCMInterface(BOARD_t* pBoard,
                                             IFX_boolean_t hMaster);
IFX_return_t BOARD_Easy80800_GetCapabilities(BOARD_t* pBoard,
                                             IFX_char_t* pPath);
IFX_void_t BOARD_Easy80800_RemoveBoard(BOARD_t* pBoard);
IFX_return_t BOARD_Easy80800_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg);
...
IFX_return_t BOARD_Easy80800_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath)
{
   ...
   if (BOARD_Easy80800_PrepareNames(pBoard) == IFX_ERROR)
      return IFX_ERROR;

   /** Startup mapping table */
   pBoard->pChStartupMapping = STARTUP_MAPPING_VMMC;

   if (IFX_ERROR == BOARD_Easy80800_GetCapabilities(pBoard, pPath))
   {
      return IFX_ERROR;
   }
...
IFX_return_t BOARD_Easy80800_PrepareNames(BOARD_t* pBoard)
{
   ...
   strncpy(pBoard->pszChDevName, sCH_DEV_NAME_VMMC, strlen(sCH_DEV_NAME_VMMC));
...
IFX_return_t BOARD_Easy80800_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg)
{
   ...
   if (BOARD_Easy80800_PrepareBoard(pBoard, pProgramArg->sPathToDwnldFiles) == IFX_ERROR)
   {
       return IFX_ERROR;
   }
   ...
   /* Initialize the PCM Interface. Must do this, otherwise access to
      extension board or FXO w will not work. */
   if (BOARD_Easy80800_CfgPCMInterface(pBoard,
...
IFX_return_t BOARD_Easy80800_Register(BOARD_t* pBoard)
{
   ...
   pBoard->pszBoardName = BOARD_NAME_EASY80800;
   pBoard->Init = BOARD_Easy80800_Init;
   pBoard->RemoveBoard = BOARD_Easy80800_RemoveBoard;

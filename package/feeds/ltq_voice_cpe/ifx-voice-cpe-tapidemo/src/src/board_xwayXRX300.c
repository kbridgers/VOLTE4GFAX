...
/**
   \file board_xwayXRX300.c
   \date 2011-10-26
   \brief Interface implementation for XWAY(TM) XRX300 Evaluation Boards.
...
/** Name of board */
static const char* BOARD_NAME_XWAY_XRX300 = "XWAY_XRX300";
...
/** GPIO port name */
/* Currently we don't need GPIO port name */
/* static IFX_char_t* GPIO_PORT_NAME = "/dev/amazon_s-port"; */
...
/* ============================= */
IFX_return_t BOARD_XwayXRX300_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath);
IFX_return_t BOARD_XwayXRX300_PrepareNames(BOARD_t* pBoard);
IFX_return_t BOARD_XwayXRX300_CfgPCMInterface(BOARD_t* pBoard,
                                             IFX_boolean_t hMaster);
IFX_return_t BOARD_XwayXRX300_GetCapabilities(BOARD_t* pBoard,
                                             IFX_char_t* pPath);
IFX_void_t BOARD_XwayXRX300_RemoveBoard(BOARD_t* pBoard);
IFX_return_t BOARD_XwayXRX300_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg);
...
IFX_return_t BOARD_XwayXRX300_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath)
{
   ...
   if (BOARD_XwayXRX300_PrepareNames(pBoard) == IFX_ERROR)
      return IFX_ERROR;

   /** Startup mapping table */
   pBoard->pChStartupMapping = STARTUP_MAPPING_VMMC;

   if (IFX_ERROR == BOARD_XwayXRX300_GetCapabilities(pBoard, pPath))
   {
      return IFX_ERROR;
   }
...
IFX_return_t BOARD_XwayXRX300_PrepareNames(BOARD_t* pBoard)
{
   ...
   {
      strncat(pBoard->pszCtrlDevName, tmp_buf, 1);
   ...
   memset(tmp_buf, 0, 10);
   snprintf(tmp_buf, 1, "%d", (int) pBoard->nID - 1);
   if (DEV_NAME_LEN > (strlen(tmp_buf) + strlen(pBoard->pszSystemDevName)))
   {
      strncat(pBoard->pszSystemDevName, tmp_buf, 1);
                                             ...
                                             IFX_boolean_t hMaster)
{
   ...
   return IFX_SUCCESS;
} /* BOARD_XwayXRX300_CfgPCMInterface() */
...
IFX_return_t BOARD_XwayXRX300_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg)
{
   ...
   if (BOARD_XwayXRX300_PrepareBoard(pBoard, pProgramArg->sPathToDwnldFiles) == IFX_ERROR)
   {
       return IFX_ERROR;
   }
   ...
   if (pBoard->nMaxPCM_Ch > 0)
   {
      ...
      if (BOARD_XwayXRX300_CfgPCMInterface(pBoard,
...
IFX_return_t BOARD_XwayXRX300_Register(BOARD_t* pBoard)
{
   ...
   pBoard->pszBoardName = BOARD_NAME_XWAY_XRX300;
   pBoard->Init = BOARD_XwayXRX300_Init;
   pBoard->RemoveBoard = BOARD_XwayXRX300_RemoveBoard;

...
/* ============================= */
   ...
   IFX_char_t* sBBD_CRAM_File_SVIP_R = "bbd_svip_r.bin";
   IFX_char_t* sBBD_CRAM_File_SVIP_S = "bbd_svip_s.bin";
   IFX_char_t* sBBD_CRAM_File_SVIP_P = "bbd_svip_p.bin";
#endif /* USE_FILESYSTEM */

/** Device names */
IFX_char_t* sCTRL_DEV_NAME_SVIP = "/dev/svip";

/** Holding device dependent data */
SVIP_IO_INIT SvipDevInitStruct = {0};
/* ============================= */
/* Local function declaration    */
/* ============================= */
IFX_return_t DEVICE_SVIP_Fillin_FW_Ptr(BOARD_TYPE_t nChipType,
                                       IFX_void_t* pDrvInit,
                                       TAPIDEMO_DEVICE_CPU_t* pCpuDevice,
                                       IFX_char_t* pPath);
IFX_void_t DEVICE_SVIP_Release_FW_Ptr(IFX_void_t* pDrvInit);
IFX_return_t DEVICE_SVIP_GetVersions(BOARD_t* pBoard);
IFX_return_t DEVICE_SVIP_Init(BOARD_t* pBoard, IFX_char_t* pPath);
IFX_void_t DEVICE_SVIP_DecodeErrnoForDrv(IFX_int32_t nCode,
                                         IFX_uint32_t nSeqConnId);
IFX_return_t DEVICE_SVIP_SetupChannel(BOARD_t* pBoard, IFX_char_t* pPath);

#ifdef TD_USE_CHANNEL_INIT
IFX_return_t DEVICE_SVIP_SetupChannelUseChInit(BOARD_t* pBoard,
                                               IFX_char_t* pPath);
...
/**
   ...
   \param pDrvInit - handle to SVIP_IO_INIT structure, should not be null
...
{
#ifdef USE_FILESYSTEM
   SVIP_IO_INIT* vinit = (SVIP_IO_INIT *) pDrvInit;
   ...
   if (vinit->pBBDbuf != IFX_NULL)
   {
      /* fw and bbd files already downloaded. If there is need to
         ...
         must be released - DEVICE_SVIP_Release_FW_Ptr()*/
   ...
   COM_RCV_FILES_GET(nChipType, ITM_RCV_FILE_BBD_FXS,
                     &vinit->pBBDbuf, (IFX_size_t*) &vinit->bbd_size);
...
/**
   ...
   \param pDrvInit - handle to SVIP_IO_INIT structure
...
IFX_return_t DEVICE_SVIP_Init(BOARD_t* pBoard, IFX_char_t* pPath)
{
...
#endif /* TD_USE_CHANNEL_INIT */
   {
      DEVICE_SVIP_Fillin_FW_Ptr(pBoard->nType, pCpuDevice->pDevDependStuff,
                                pCpuDevice, pPath);
   ...
   {
      /* Read version */
      DEVICE_SVIP_GetVersions(pBoard);
   ...
   else
   {
      ...
      TD_OS_MSecSleep(SVIP_XT16_WAIT_CALIBRATION_POLLTIME * pBoard->nMaxAnalogCh);
      /* Check if callibration end event was received for all tapi channels */
      for (i = 0; i < pBoard->nMaxAnalogCh; i++)
      {
         /* if previous step of initialiation failed then skip also this one */
         if (IFX_ERROR == pChannelTable[i])
         {
            continue;
         }
   ...
   DEVICE_SVIP_Release_FW_Ptr(pCpuDevice->pDevDependStuff);
...
/**
   ...
   \remarks
*/
IFX_return_t DEVICE_SVIP_GetErrnoNum(IFX_int32_t nFd)
{
   return IFX_SUCCESS;
}

/**
   Initialize TAPI to read cappabilities.

   \param pBoard - pointer to board
   \param pPath - path to FW files for download - not used
...
{
#ifdef TD_USE_CHANNEL_INIT
   return DEVICE_SVIP_SetupChannelUseChInit(pBoard, pPath);
#else
   return DEVICE_SVIP_SetupChannelUseDevStart(pBoard, pPath);
#endif /* TD_USE_CHANNEL_INIT */
}
...
/**
   ...
   \remarks It is necessary to call this function to have working the
    IFX_TAPI_CAP_LIST.
*/
IFX_return_t DEVICE_SVIP_SetupChannelUseChInit(BOARD_t* pBoard, IFX_char_t* pPath)
{
   IFX_return_t ret;
   IFX_TAPI_CH_INIT_t Init;
   TAPIDEMO_DEVICE_t *pDevice = IFX_NULL;
   ...
   pCpuDevice = &pDevice->uDevice.stCpu;
   /* get BBD */
   ret = DEVICE_SVIP_Fillin_FW_Ptr(pBoard->nType, pCpuDevice->pDevDependStuff,
                                   pCpuDevice, pPath);
   if (ret == IFX_ERROR)
   {
      TD_TRACE(TAPIDEMO, DBG_LEVEL_HIGH, TD_CONN_ID_INIT,
...
#endif /* TAPI_VERSION4 */
   {
      ...
      ret = TD_IOCTL(pBoard->nDevCtrl_FD, IFX_TAPI_CH_INIT,
               (IFX_int32_t) &Init, nDevTmp, TD_CONN_ID_INIT);
      if (IFX_SUCCESS != ret)
      {
         if (0 != nDevTmp)
         {
...
/**
   ...
   \remarks It is necessary to call this function to have working the
    IFX_TAPI_CAP_LIST.
*/
IFX_return_t DEVICE_SVIP_SetupChannelUseDevStart(BOARD_t* pBoard, IFX_char_t* pPath)
{
...
#endif /* TAPI_VERSION4 */
   {
      ...
      oParamConfig.nMode = IFX_TAPI_INIT_MODE_VOICE_CODER;
      ...
      ret = TD_IOCTL(pBoard->nDevCtrl_FD, IFX_TAPI_DEV_START, &oParamConfig,
      ...
      case 'R':
         ...
         break;
      case 'S':
         pCpuDevice->pszBBD_CRAM_File[0] = sBBD_CRAM_File_SVIP_S;
         break;
      case 'P':
         pCpuDevice->pszBBD_CRAM_File[0] = sBBD_CRAM_File_SVIP_P;
         break;
      default:
         TD_TRACE(TAPIDEMO, DBG_LEVEL_HIGH, TD_CONN_ID_INIT,
               ("Err, Unknown SLIC type. (File: %s, line: %d)\n",
      ...
      oBBD.buf = ((SVIP_IO_INIT*)pCpuDevice->pDevDependStuff)->pBBDbuf;
      oBBD.size = ((SVIP_IO_INIT*)pCpuDevice->pDevDependStuff)->bbd_size;
#ifdef TAPI_VERSION4
      oBBD.dev = i;
      nDevTmp = oBBD.dev;
#endif /* TAPI_VERSION4 */
...
IFX_return_t DEVICE_SVIP_Register(TAPIDEMO_DEVICE_CPU_t* pCpuDevice)
{
   ...
   pCpuDevice->Version = DEVICE_SVIP_GetVersions;
   pCpuDevice->DecodeErrno = DEVICE_SVIP_DecodeErrnoForDrv;
   pCpuDevice->GetErrno = DEVICE_SVIP_GetErrnoNum;

   pCpuDevice->FillinFwPtr = DEVICE_SVIP_Fillin_FW_Ptr;
   pCpuDevice->ReleaseFwPtr = DEVICE_SVIP_Release_FW_Ptr;

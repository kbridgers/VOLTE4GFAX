...
/**
   \file board_easy508xx.c
   \date 2006-10-01
   \brief Interface implementation for EASY508xx boards.
...
/** Name of board */
static const char* BOARD_NAME_EASY508XX = "EASY508XX";
...
/* ============================= */
IFX_return_t BOARD_Easy508XX_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath);
IFX_return_t BOARD_Easy508XX_PrepareNames(BOARD_t* pBoard);
IFX_return_t BOARD_Easy508XX_CfgPCMInterface(BOARD_t* pBoard,
                                             IFX_boolean_t hMaster);
IFX_return_t BOARD_Easy508XX_GetCapabilities(BOARD_t* pBoard,
                                             IFX_char_t* pPath);
IFX_void_t BOARD_Easy508XX_RemoveBoard(BOARD_t* pBoard);
IFX_return_t BOARD_Easy508XX_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg);
...
IFX_return_t BOARD_Easy508XX_PrepareBoard(BOARD_t* pBoard, IFX_char_t* pPath)
{
   ...
   if (BOARD_Easy508XX_PrepareNames(pBoard) == IFX_ERROR)
      return IFX_ERROR;

   /** Startup mapping table */
   pBoard->pChStartupMapping = STARTUP_MAPPING_VMMC;

   if (IFX_ERROR == BOARD_Easy508XX_GetCapabilities(pBoard, pPath))
   {
      return IFX_ERROR;
   }
                                             ...
                                             IFX_boolean_t hMaster)
{
   ...
   return IFX_SUCCESS;
} /* BOARD_Easy508XX_CfgPCMInterface() */
...
IFX_return_t BOARD_Easy508XX_Init(BOARD_t* pBoard, PROGRAM_ARG_t* pProgramArg)
{
   ...
   if (BOARD_Easy508XX_PrepareBoard(pBoard, pProgramArg->sPathToDwnldFiles) == IFX_ERROR)
   {
       return IFX_ERROR;
   }
   ...
   if (pBoard->nMaxPCM_Ch > 0)
   {
      ...
      if (BOARD_Easy508XX_CfgPCMInterface(pBoard,
...
IFX_return_t BOARD_Easy508XX_Register(BOARD_t* pBoard)
{
   ...
   pBoard->pszBoardName = BOARD_NAME_EASY508XX;
   pBoard->Init = BOARD_Easy508XX_Init;
   pBoard->RemoveBoard = BOARD_Easy508XX_RemoveBoard;

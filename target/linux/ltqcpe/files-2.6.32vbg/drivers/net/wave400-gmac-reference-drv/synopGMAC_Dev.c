...
WAVE400_STATIC_INLINE void synopGMAC_tx_checksum_offload_ipv4hdr(synopGMACdevice *gmacdev, DmaDesc *desc);
WAVE400_STATIC_INLINE void synopGMAC_tx_checksum_offload_tcponly(synopGMACdevice *gmacdev, DmaDesc *desc);

/********************************************************************
* PHY related database
* (similar to wave400 u-boot implementation)
* HowTo Add PHY (instruction here):
* - add new phy_info struct (see phy_info_wave400_22161):
*   config - reset phy, followed by write required phy setting (don't trust default)
*   startup - check phy link and setting params after the negitiation stage
*   shutdown - anything to do when driver is down?
*   disconnect - do stuff when cable disconnect/connect detected
* - add the new struct to the phy_info struct array (phy_info_array)
* - implement the handlers for the new phy (as they are called from the phy_info struct)
*********************************************************************/
...
s32 mii_parse_link_22161(u16 data, synopGMACdevice * gmacdev);
s32 mii_parse_sr_22161(u16 data, synopGMACdevice * gmacdev);
s32 mii_parse_sr_1378e(u16 data, synopGMACdevice * gmacdev);
s32 synopGMAC_check_phy_discon_22161(u16 data, synopGMACdevice * gmacdev);
s32 synopGMAC_check_phy_discon_1378e(u16 data, synopGMACdevice * gmacdev);


typedef struct gphy
{
	u32	res000[4];  /* skip it */
	u32	addReg;
	u32	dataReg;
}gphy_t;
...
struct phy_cmd {
    u32 mii_reg;
    u32 mii_data;
    s32 (*funct) (u16 mii_reg, synopGMACdevice * gmacdev);
};
...
struct phy_info {
    ...
    /* Called when cable disconnect detected */
    struct phy_cmd *disconnect;
};

struct phy_info phy_info_wave400_22161 =  {
	0x22161,
	"WAVE400/Generic PHY 22161 (KS8721)",
	4,
	(struct phy_cmd[]) { /* config */
		{PHY_CONTROL_REG, PHY_BMCR_RESET, NULL},
		{PHY_CONTROL_REG, PHY_BMCR_AUTON|PHY_BMCR_RST_NEG, NULL},
		{miim_end,}
	},
	(struct phy_cmd[]) { /* startup */
		{PHY_STATUS_REG, miim_read, &mii_parse_link_22161/*wait link/autoneg complete*/},
		{PHY_100BTX_CTRL_REG, miim_read, &mii_parse_sr_22161/*set duplex/speed*/},
		{miim_end,}
	},
	(struct phy_cmd[]) { /* shutdown */
		{miim_end,}
	...
	(struct phy_cmd[]) { /* disconnect */
		{PHY_STATUS_REG, miim_read, &mii_parse_link_22161/*wait link/autoneg complete*/},
		{PHY_100BTX_CTRL_REG, miim_read, &synopGMAC_check_phy_discon_22161/*test autoneg results*/},
		{miim_end,}
	}
};


struct phy_info phy_info_wave400_1378e =  {
	0x1378e/*intel LXT971A on external board*/,
	"WAVE400/Generic PHY 2 (LX971)",
	4,
	(struct phy_cmd[]) { /* config */
		{PHY_CONTROL_REG, PHY_BMCR_RESET, NULL},
		{PHY_CONTROL_REG, PHY_BMCR_AUTON|PHY_BMCR_RST_NEG, NULL},
	...
	(struct phy_cmd[]) { /* startup */
		{PHY_STATUS_REG, miim_read, &mii_parse_link_22161/*wait link/autoneg complete*/},
		{PHY_100BTX_CTRL_REG, miim_read, &mii_parse_sr_1378e/*set duplex/speed*/},
		{miim_end,}
	},
	(struct phy_cmd[]) { /* shutdown */
		{miim_end,}
	...
	(struct phy_cmd[]) { /* disconnect */
		{PHY_STATUS_REG, miim_read, &mii_parse_link_22161/*wait link/autoneg complete*/},
		{17/*PHY_STATUS_2_REG*/, miim_read, &synopGMAC_check_phy_discon_1378e/*test autoneg results*/},
		{miim_end,}
	}
};

struct phy_info *phy_info_array[] = {
	&phy_info_wave400_22161,
	&phy_info_wave400_1378e,
	NULL
};
...
s32 synopGMAC_read_phy_reg(u32 *RegBase,u32 PhyBase, u32 RegOffset, u16 * data)
{
    ...
    addr = addr | GmiiBusy |GmiiCsrClk3 ; //Gmii busy bit
...
#if 0
addr = ((PhyBase << GmiiDevShift/*11*/) & GmiiDevMask/*f800*/) | ((RegOffset << GmiiRegShift/*6*/) & GmiiRegMask/*7c0*/) | GmiiWrite/*2*/ |0xb/*GmiiCsrClk3*//*c*/;

addr = addr | GmiiBusy ; //set Gmii clk to 20-35 Mhz and Gmii busy bit
#else
addr = 0x080b | ((RegOffset << GmiiRegShift/*6*/) & GmiiRegMask/*7c0*/);
#endif
TR0("addr = 0x%x \n",addr);
...
data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaBusMode);
	...
	//TR0("3.3, DmaBase=0x%p, DmaBusMode=0x%08x\n",(u32 *)gmacdev->DmaBase,DmaBusMode);
	data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaBusMode);
	//TR0("DATA after Reset = %08x\n",data);
...
u32 synopGMAC_get_filter(synopGMACdevice * gmacdev)
{
	return synopGMACReadReg((u32 *)gmacdev->MacBase, GmacFrameFilter);
}
...
s32 synopGMAC_dump_phy_reg(u32 *RegBase,u32 PhyBase, u32 RegOffset, u16 * data)
{
u32 addr;
u32 loop_variable;
addr = ((PhyBase << GmiiDevShift) & GmiiDevMask) | ((RegOffset << GmiiRegShift) & GmiiRegMask);
...
void dump_registers(synopGMACdevice * gmacdev)
{
    u16 data;

	TR0("GmacConfig\n");
    synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacConfig);
	TR0("GmacFrameFilter\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacFrameFilter);
	TR0("GmacHashHigh\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacHashHigh);
	TR0("GmacHashLow\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacHashLow);
	TR0("GmacFlowControl\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacFlowControl);
	TR0("GmacVlan\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacVlan);
	TR0("GmacVersion\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacVersion);
	TR0("GmacWakeupAddr\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacWakeupAddr);
	TR0("GmacPmtCtrlStatus\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacPmtCtrlStatus);
	TR0("GmacInterruptStatus\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacInterruptStatus);
	TR0("GmacInterruptMask\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacInterruptMask);
	TR0("GmacAddr0High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr0High);
	TR0("GmacAddr0Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr0Low);
	TR0("GmacAddr1High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr1High);
	TR0("GmacAddr1Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr1Low);
	TR0("GmacAddr2High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr2High);
	TR0("GmacAddr2Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr2Low);
	TR0("GmacAddr3Highx\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr3High);
	TR0("GmacAddr3Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr3Low);
	TR0("GmacAddr4High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr4High);
	TR0("GmacAddr4Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr4Low);
	TR0("GmacAddr5High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr5High);
	TR0("GmacAddr5High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr5High);
	TR0("GmacAddr5Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr5Low);
	TR0("GmacAddr6High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr6High);
	TR0("GmacAddr6Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr6Low);
	TR0("GmacAddr7High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr7High);
	TR0("GmacAddr7Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr7Low);
	TR0("GmacAddr8High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr8High);
	TR0("GmacAddr8Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr8Low);
	TR0("GmacAddr9High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr9High);
	TR0("GmacAddr9Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr9Low);
	TR0("GmacAddr10High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr10High);
	TR0("GmacAddr10Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr10Low);
	TR0("GmacAddr11High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr11High);
	TR0("GmacAddr11Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr11Low);
	TR0("GmacAddr12High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr12High);
	TR0("GmacAddr12Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr12Low);
	TR0("GmacAddr13High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr13High);
	TR0("GmacAddr13Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr13Low);
	TR0("GmacAddr14High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr14High);
	TR0("GmacAddr14Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr14Low);
	TR0("GmacAddr15High\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr15High);
	TR0("GmacAddr15Low\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacAddr15Low);
	TR0("GmacTSControl\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSControl);
	TR0("GmacTSSubSecIncr\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSSubSecIncr);
	TR0("GmacTSHigh\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSHigh);
	TR0("GmacTSLow\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSLow);
	TR0("GmacTSHighUpdate\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSHighUpdate);
	TR0("GmacTSLowUpdate\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSLowUpdate);
	TR0("GmacTSAddend\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSAddend);
	TR0("GmacTSTargetTimeHigh\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSTargetTimeHigh);
	TR0("GmacTSTargetTimeLow\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSTargetTimeLow);
	TR0("GmacTSHighWord\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSHighWord);
	TR0("GmacTSStatus\n");
	synopGMACDumpReg((u32 *)gmacdev->MacBase, GmacTSStatus);
    /*DMA*/
	TR0("DmaBusMode\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaBusMode);
	TR0("DmaTxPollDemand\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaTxPollDemand);
	TR0("DmaRxPollDemand\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaRxPollDemand);
	TR0("DmaRxBaseAddr\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaRxBaseAddr);
	TR0("DmaTxBaseAddr\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaTxBaseAddr);
	TR0("DmaStatus\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaStatus);
	TR0("DmaControl\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaControl);
	TR0("DmaInterrupt\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaInterrupt);
	TR0("DmaMissedFr\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaMissedFr);
	TR0("DmaTxCurrDesc\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaTxCurrDesc);
	TR0("DmaRxCurrDesc\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaRxCurrDesc);
	TR0("DmaTxCurrAddr\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaTxCurrAddr);
	TR0("DmaRxCurrAddr\n");
	synopGMACDumpReg((u32 *)gmacdev->DmaBase, DmaRxCurrAddr);
    /*PHY*/
	TR0("PHY_CONTROL_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTROL_REG,&data);
	TR0("PHY_STATUS_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_STATUS_REG,&data);
	TR0("PHY_ID_HI_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_ID_HI_REG,&data);
	TR0("PHY_ID_LOW_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_ID_LOW_REG,&data);
	TR0("PHY_AN_ADV_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_AN_ADV_REG,&data);
	TR0("PHY_LNK_PART_ABl_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_LNK_PART_ABl_REG,&data);
	TR0("PHY_AN_EXP_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_AN_EXP_REG,&data);
	TR0("PHY_AN_NXT_PAGE_TX_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_AN_NXT_PAGE_TX_REG,&data);
	TR0("PHY_LNK_PART_NXT_PAGE_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_LNK_PART_NXT_PAGE_REG,&data);
	TR0("PHY_1000BT_CTRL_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_1000BT_CTRL_REG,&data);
	TR0("PHY_1000BT_STATUS_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_1000BT_STATUS_REG,&data);
	TR0("PHY_SPECIFIC_CTRL_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_SPECIFIC_CTRL_REG,&data);
	TR0("PHY_SPECIFIC_STATUS_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_SPECIFIC_STATUS_REG,&data);
	TR0("PHY_INTERRUPT_ENABLE_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_INTERRUPT_ENABLE_REG,&data);
	TR0("PHY_INTERRUPT_STATUS_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_INTERRUPT_STATUS_REG,&data);
	TR0("PHY_EXT_PHY_SPC_CTRL\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_PHY_SPC_CTRL,&data);
	TR0("PHY_RX_ERR_COUNTER\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RX_ERR_COUNTER,&data);
	TR0("PHY_EXT_ADDR_CBL_DIAG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_ADDR_CBL_DIAG,&data);
	TR0("PHY_LED_CONTROL\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_LED_CONTROL,&data);
	TR0("PHY_MAN_LED_OVERIDE\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_MAN_LED_OVERIDE,&data);
	TR0("PHY_EXT_PHY_SPC_CTRL2\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_PHY_SPC_CTRL2,&data);
	TR0("PHY_EXT_PHY_SPC_STATUS\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_EXT_PHY_SPC_STATUS,&data);
	TR0("PHY_CBL_DIAG_REG\n");
	synopGMAC_dump_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CBL_DIAG_REG,&data);
    /*Mac Management Counters (MMC) -- needed???*/
...
s32 synopGMAC_mac_init(synopGMACdevice * gmacdev)
{
	...
	if(gmacdev->DuplexMode == FULLDUPLEX){
		...
		if(gmacdev->Speed == SPEED1000) {
		    TR0("SPEED1000, not like uboot !!, DuplexMode = %d\n",gmacdev->DuplexMode);
			synopGMAC_select_gmii(gmacdev);
		}
		else {
		    TR0("SPEED100, like uboot, DuplexMode = %d\n",gmacdev->DuplexMode);
...
s32 mii_parse_link_22161(u16 data, synopGMACdevice * gmacdev)
{
	...
	loop_count = /*DEFAULT_LOOP_VARIABLE*/100000;
	TR0("%s: gmacdev->MacBase = 0x%08x, gmacdev->PhyBase = 0x%08x\n",__FUNCTION__,gmacdev->MacBase,gmacdev->PhyBase);
	
	/*status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_STATUS_REG, &data); //!!! move it to config handler, add handler to struct !
	if(status)
		return status;
	if ((data & Mii_Link) && !(data & Mii_AutoNegCmplt)) {
		TR0("Waiting for PHY auto negotiation to complete \n");
	}
	else if (data & Mii_Link) {
		TR0("link is up, goto link \n");
	    goto link;
    }*/
	while(loop_count-- > 0)
	{
		...
		if(status)	 {
			TR0("Error reading Autonegotiation status\n");
			return status;
        }

	    if((data & Mii_AutoNegCmplt) != 0){
			TR0("Autonegotiation Complete\n");
			break;
		}
	}
	TR0("PHY_STATUS_REG: data = 0x%x, loop_count = %d\n",data,loop_count);
...
s32 mii_parse_sr_22161(u16 data, synopGMACdevice * gmacdev)
{
	...
	TR0("PHY_100BTX_CTRL_REG: data = 0x%x\n",data);
	gmacdev->DuplexMode = (data & Mii_phy100_status_full_duplex)  ? FULLDUPLEX: HALFDUPLEX ;
    if(data & Mii_phy_status_speed_1000)
        gmacdev->Speed      =   SPEED1000;
    else if(data & Mii_phy100_status_speed_100h)
	...
	if(gmacdev->DuplexMode == HALFDUPLEX){
		...
		TR0("PHY_CONTROL_REG: data = 0x%x, status = 0x%08x\n",data,status);
		if(status)
			return status;
...
s32 mii_parse_sr_1378e(u16 data, synopGMACdevice * gmacdev)
{
	TR0("gmacdev->MacBase = 0x%08x, gmacdev->PhyBase = 0x%08x\n",gmacdev->MacBase,gmacdev->PhyBase);

	TR0("PHY_STATUS_2_REG: data = 0x%x\n",data);
	gmacdev->DuplexMode = (data & 0x0200)  ? FULLDUPLEX: HALFDUPLEX ;
...
*/
struct phy_info *get_phy_info(synopGMACdevice * gmacdev)
{
	int i;
	...
	for (i = 0; phy_info_array[i]; i++) {
		if (phy_info_array[i]->id == (phy_ID >> phy_info_array[i]->shift)) {
			theInfo = phy_info_array[i];
			break;
		}
	}

	if (theInfo == NULL) {
		TR0("PHY id %x is not supported!\n", phy_ID);
		return NULL;
	} else {
		TR0("PHY is %s (%x) (shifted %x)\n", theInfo->name, phy_ID, (phy_info_array[i]->id));
	}
...
*/
void phy_run_commands(synopGMACdevice * gmacdev, struct phy_cmd *cmd)
{
	int i;
	u16 data;
	s32 status = -ESYNOPGMACNOERR;		
	...
	for (i = 0; cmd->mii_reg != miim_end; i++) {
		if (cmd->mii_data == miim_read) {
	        status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, cmd->mii_reg, &data);
			TR0("read PHY reg 0x%08x, data = 0x%x\n",cmd->mii_reg,data);
			plat_delay(100000);	

			if (cmd->funct != NULL)
				(*(cmd->funct)) (data, gmacdev);

		} else {
			if (cmd->funct != NULL)
				data = (*(cmd->funct)) (cmd->mii_reg, gmacdev);
			else
				data = cmd->mii_data;

			TR0("write PHY addr 0x%08x  data = 0x%x\n",cmd->mii_reg,data);
			synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,cmd->mii_reg, data);
...
* for wave400, phy_info_wave400 is used.
*/
s32 synopGMAC_phy_init (synopGMACdevice * gmacdev) 
{	
	struct phy_info *curphy;
	s32 status = -ESYNOPGMACNOERR;		

	TR0("in synopGMAC_phy_init\n");
	/* Get the cmd structure corresponding to the attached
	* PHY */
	curphy = get_phy_info(gmacdev);
	...
	phy_run_commands(gmacdev, curphy->config);
...
{	
	...
	TR0("in synopGMAC_check_phy_init\n");
	/* Get the cmd structure corresponding to the attached
	* PHY */
	curphy = get_phy_info(gmacdev);
	...
	phy_run_commands(gmacdev, curphy->startup);
...
{	
	...
	/*getting here - link is up already*/
	TR0("PHY_100BTX_CTRL_REG: data = 0x%x\n",data);
	DuplexMode_new = (data & Mii_phy100_status_full_duplex)  ? FULLDUPLEX: HALFDUPLEX ;
	...
	if (DuplexMode_new != gmacdev->DuplexMode) {
		gmacdev->DuplexMode = DuplexMode_new;
        if (gmacdev->DuplexMode == FULLDUPLEX/*was half*/)
            synopGMAC_set_full_duplex(gmacdev);
        else
            synopGMAC_set_half_duplex(gmacdev);
    }
    if (Speed_new != gmacdev->Speed) {

		if(gmacdev->Speed == SPEED1000)
			Speed_old = SPEED1000;
		gmacdev->Speed = Speed_new;
		if(gmacdev->Speed == SPEED1000)
			synopGMAC_select_gmii(gmacdev);
		else if (Speed_old == SPEED1000/*need to change*/)
			synopGMAC_select_mii(gmacdev);
     }
...
{	
	...
	/*getting here - link is up*/	
	TR0("PHY_STATUS_2_REG: data = 0x%x\n",data);
	DuplexMode_new  = (data & 0x0200)  ? FULLDUPLEX: HALFDUPLEX ;
    /*not support speed 1000*/
	if(data & 0x4000)
		Speed_new      =   SPEED100;
	else
...
s32 synopGMAC_check_phy_discon (synopGMACdevice * gmacdev) 
{
	struct phy_info *curphy;
	s32 status = -ESYNOPGMACNOERR;		
	...
	* PHY */
	...
	phy_run_commands(gmacdev, curphy->disconnect);
...
s32 synopGMAC_get_mac_addr(synopGMACdevice *gmacdev, u32 MacHigh, u32 MacLow, u8 *MacAddr)
{
	...
	MacAddr[4] = (data)        & 0xff;
	TR0("%s called, mac = %x\n",__FUNCTION__,MacAddr[5]);
	TR0("%s called, %x\n",__FUNCTION__,MacAddr[4]);
	...
	MacAddr[0] = (data )      & 0xff;
	TR0("%s called, %x\n",__FUNCTION__,MacAddr[3]);
	TR0("%s called, %x\n",__FUNCTION__,MacAddr[2]);
	TR0("%s called, %x\n",__FUNCTION__,MacAddr[1]);
...
TR0("gmacdev->MacBase=0x%08x, gmacdev->DmaBase=0x%08x, gmacdev->PhyBase=0x%08x",gmacdev->MacBase,gmacdev->DmaBase,gmacdev->PhyBase);
...
/**
  ...
  *	- buffer1 (and buffer2->removed) set to 0 for ring mode of operation. (note)
  *	- data1 and data2 set to 0. (note)
  * @param[in] pointer to DmaDesc structure.
  * @param[in] whether end of ring
  * \return void.
  * \note Initialization of the buffer1, data1, (buffer2,data2->removed) and status are not done here. This only initializes whether one wants to use this descriptor
  * in chain mode or ring mode. (For chain mode of operation the buffer2 and data2 are programmed before calling this function ->removed).
  */
WAVE400_INLINE void synopGMAC_rx_desc_init_ring(DmaDesc *desc, bool last_ring_desc)
{
	desc->status = 0;
	...
	desc->data1 = 0;
   	if (init_desc)
   	  TR("desc_init: buffer1 = %p, data1 = %p \n",&desc->buffer1,&desc->data1);
	return;
}
/**
  ...
  * in chain mode or ring mode. (For chain mode of operation the buffer2 and data2 are programmed before calling this function ->removed).
  */
WAVE400_INLINE void synopGMAC_tx_desc_init_ring(DmaDesc *desc, bool last_ring_desc)
{
	#ifdef ENH_DESC
...
/**
  ...
  * 	- Status field is initialized to 0.
  *	- EndOfRing set for the last descriptor.
  *	- buffer1 (and buffer2 ->removed) set to 0.
  *	- data1 (and data2 ->removed) set to 0.
  * @param[in] pointer to DmaDesc structure.
  * @param[in] whether end of ring
  * \return void.
  */
...
{
    TR0("set (u32)gmacdev->RxDescDma as 0x%08x",(u32)gmacdev->RxDescDma);
...
{
    TR0("set (u32)gmacdev->TxDescDma as 0x%08x",(u32)gmacdev->TxDescDma);
...
/** 
  ...
  */
WAVE400_STATIC_INLINE bool synopGMAC_is_desc_owned_by_dma(DmaDesc *desc)
{
    return ((desc->status & DescOwnByDma) == DescOwnByDma );
}

u32 synopGMAC_print_desc_own(DmaDesc *desc)
{
    TR0("own bit = %08x \n",(desc->status & DescOwnByDma));
    return (u32)(desc->status & 0x80000000/*DescOwnByDma*/);
}
u32 synopGMAC_print_desc_empty(DmaDesc *desc)
{
    TR0("len = %08x \n",(desc->length & DescSize1Mask));
    return (u32)(desc->length & 0x000007ff/*DescSize1Mask*/);
}

void synopGMAC_print_desc(DmaDesc *desc)
{
    TR0("buffer1 = %08x, data1 = %08x , length = %08x \n",desc->buffer1,desc->data1,desc->length);
}
...
/**
  ...
  */
WAVE400_INLINE bool synopGMAC_is_desc_valid(u32 status)
{
...
/**
  ...
  * If the buffer1 (and buffer2 length-> removed) is zero in ring mode descriptor is empty.
  * (In chain mode buffer2 length is 0 but buffer2 itself contains the next descriptor address ->removed).
  * @param[in] pointer to DmaDesc structure.
  * \return returns true if descriptor is empty, false if not empty.
  */
WAVE400_STATIC_INLINE bool synopGMAC_is_desc_empty(DmaDesc *desc)
{
    //if both the buffer1 length (and buffer2 length-> removed) is zero desc is empty
...
/**
  ...
  */
WAVE400_INLINE bool synopGMAC_is_rx_desc_valid(u32 status)
{
...
/**
  ...
  */
WAVE400_INLINE bool synopGMAC_is_tx_aborted(u32 status)
{
...
/**
  ...
  */
WAVE400_INLINE bool synopGMAC_is_tx_carrier_error(u32 status)
{
...
/**
  ...
  */
WAVE400_INLINE u32 synopGMAC_get_tx_collision_count(u32 status)
{
...
/**
  ...
  * \return returns true if error else returns false.
  */
WAVE400_INLINE bool synopGMAC_is_rx_frame_collision(u32 status)
{
//bool synopGMAC_dma_rx_collisions(u32 status)
...
/**
  ...
  * \return returns true if error else returns false.
  */
WAVE400_INLINE bool synopGMAC_is_rx_crc(u32 status)
{
...
/**
  ...
  * \return returns true if error else returns false.
  */
WAVE400_INLINE bool synopGMAC_is_frame_dribbling_errors(u32 status)
{
...
/**
  ...
  * \return returns true if error else returns false.
  */
WAVE400_INLINE bool synopGMAC_is_rx_frame_length_errors(u32 status)
{
...
/**
  ...
  */
WAVE400_STATIC_INLINE bool synopGMAC_is_last_rx_desc(synopGMACdevice * gmacdev,DmaDesc *desc)
{
...
/**
  ...
  * \note This function should not be called before initializing the descriptor using synopGMAC_desc_init().
  */
WAVE400_STATIC_INLINE bool synopGMAC_is_last_tx_desc(synopGMACdevice * gmacdev,DmaDesc *desc)
{
#ifdef ENH_DESC
	return (((desc->status & TxDescEndOfRing) == TxDescEndOfRing)/* || ((u32)gmacdev->TxDesc == desc->data2)*/);
//	return (((desc->status & TxDescEndOfRing) == TxDescEndOfRing) || ((u32)gmacdev->TxDesc == desc->data2)); arad test
#else
...
/**
  ...
  * (dma-able address of buffer2, length of buffer2, virtural adddress of buffer2 ->removed).
  * @param[in]  pointer to DmaDesc structure.
  ...
  * (@param[out] dma-able address of buffer2 ->removed).
  * (@param[out] length of buffer2 ->removed).
  * (@param[out] virtual address of buffer2 ->removed).
  * \return returns void.
  */
WAVE400_INLINE void synopGMAC_get_desc_data(DmaDesc * desc, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1)
{
	if(Status != 0)   
		*Status = desc->status;
...
void synopGMAC_clear_desc_data(DmaDesc * desc)
{
    desc->status = 0;
    desc->buffer1 = 0;
    desc->length = 0;
...
/**
  ...
  */
s32 synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1,
...
/**
  ...
  * @param[out] length of buffer1 (Max is 2048).
  * @param[out] virtual pointer for buffer1.
  * (@param[out] Dma-able buffer2 pointer ->removed).
  * (@param[out] length of buffer2 (Max is 2048) ->removed).
  * (@param[out] virtual pointer for buffer2 ->removed).
  * @param[out] u32 data indicating whether the descriptor is in ring mode or chain mode.
  * \return returns present tx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1)
...
#endif
#if 1//eliminate function calls
	bool last_rx_desc;
#endif
	
	if(synopGMAC_is_desc_owned_by_dma(txdesc)) {
//		TR0("tx get desc_owned_by_dma \n");
		return -1; //!!!??? what if we miss interrupts? should wait in a loop? or search for old packets to free?
	}
	if(synopGMAC_is_desc_empty(txdesc)) {
		TR("desc_empty, BusyTxDesc = %d \n",gmacdev->BusyTxDesc);
#ifdef USE_BUSY
      if(txnext == txover)
#endif
		return -1;
...
//	TR0("dec, BusyTxDesc = %d \n",gmacdev->BusyTxDesc);
	...
	if(Data1 != 0)
		*Data1 = txdesc->data1;
#if 1//eliminate function calls
	last_rx_desc = synopGMAC_is_last_tx_desc(gmacdev,txdesc);
	gmacdev->TxBusy     = last_rx_desc ? 0 : txover + 1;
#else
	gmacdev->TxBusy     = synopGMAC_is_last_tx_desc(gmacdev,txdesc) ? 0 : txover + 1;
#endif

	TR("%02d %08x %08x %08x %08x %08x\n",txover,(u32)txdesc,txdesc->status,txdesc->length,txdesc->buffer1,txdesc->data1);
#if 1//eliminate function calls
	gmacdev->TxBusyDesc = last_rx_desc ? gmacdev->TxDesc : (txdesc + 1);
	synopGMAC_tx_desc_init_ring(txdesc, last_rx_desc);
#else
	gmacdev->TxBusyDesc = synopGMAC_is_last_tx_desc(gmacdev,txdesc) ? gmacdev->TxDesc : (txdesc + 1);
	synopGMAC_tx_desc_init_ring(txdesc, synopGMAC_is_last_tx_desc(gmacdev,txdesc));
#endif
...
/**
  ...
  * (@param[in] Dma-able buffer2 pointer ->removed).
  * (@param[in] length of buffer2 (Max is 2048) ->removed).
  * (@param[in] virtual pointer for buffer2 ->removed).
  ...
  * \return returns present tx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_set_tx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1 ,u32 offload_needed)
{
	u32  txnext      = gmacdev->TxNext;
	DmaDesc * txdesc = gmacdev->TxNextDesc;
#if 1
	u32  txbusy      = gmacdev->TxBusy;
#endif
#if 1//eliminate function calls
	bool last_rx_desc;
#endif
#ifdef WAVE400_TEST_PARAM_SETTING
	/******** TEST **********/
	static u32 count_diff_op=0;
	static u32 count_diff_0=0;
	static u32 count_diff_1=0;
	static u32 diff0=0;
	static u32 diff1=0;
	/********** END ********/
#endif
	...
	if ((txdesc->status & DescOwnByDma) == DescOwnByDma) {
		TR0("!!! ERROR DescOwnByDma found, do not interfare\n");
#ifdef WAVE400_DEBUG_OWN_MISMATCH
		synopGMAC_print_tx_desc_debug(gmacdev);
#endif
		return -2;
	}
	
	(gmacdev->BusyTxDesc)++; //busy tx descriptor is incremented by one as it will be handed over to MAC now
	//TR0("it is Not chained desc \n");
	...
	#endif
	//if ((txdesc->status & DescOwnByDma) == DescOwnByDma)
	//  TR0("!!!!!!!!!! ERROR DescOwnByDma found\n");

#ifdef WAVE400_TEST_PARAM_SETTING
	/*************************************** Start Test ********************************/
	count_diff_op++;
	diff0 = MT_RdReg(MT_LOCAL_MIPS_BASE_ADDRESS, REG_TIMER_2_COUNT_VALUE);
	diff0 = MT_RdReg(MT_LOCAL_MIPS_BASE_ADDRESS, REG_TIMER_2_COUNT_VALUE) - diff0;

	diff1 = MT_RdReg(MT_LOCAL_MIPS_BASE_ADDRESS, REG_TIMER_2_COUNT_VALUE);

	txdesc->buffer1 = Buffer1;

	diff1 = MT_RdReg(MT_LOCAL_MIPS_BASE_ADDRESS, REG_TIMER_2_COUNT_VALUE) - diff1;

	count_diff_0 += diff0;
	count_diff_1 += diff1;
	TR0(" %d: 0 = 0x%08x, 1 = 0x%08x \n",count_diff_op, (count_diff_0/count_diff_op), (count_diff_1/count_diff_op));
	/*************************************** End Test ********************************/
#else
	...
	//TR0("offload_needed \n");
	/*
	 Make sure that the OS you are running supports the IP and TCP checkusm offloading,
...
//	synopGMAC_tx_checksum_offload_tcp_pseudo(gmacdev, txdesc);
...
#if 1//eliminate function calls
	last_rx_desc = 	synopGMAC_is_last_tx_desc(gmacdev,txdesc);
	gmacdev->TxNext = last_rx_desc ? 0 : txnext + 1;
	gmacdev->TxNextDesc = last_rx_desc ? gmacdev->TxDesc : (txdesc + 1);
#else
	...
	TR("%02d %08x %08x %08x %08x %08x\n",txnext,(u32)txdesc,txdesc->status,txdesc->length,txdesc->buffer1,txdesc->data1);
	return txnext;	
}
#ifdef ENH_DESC_8W
/**
...
s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1)
{
	...
	TR("%02d %08x %08x %08x %08x %08x\n",rxnext,(u32)rxdesc,rxdesc->status,rxdesc->length,rxdesc->buffer1,rxdesc->data1/*,rxdesc->buffer2,rxdesc->data2*/);
...
#if 1//eliminate function calls
	last_rx_desc = synopGMAC_is_last_rx_desc(gmacdev,rxdesc);
	gmacdev->RxNext     = last_rx_desc ? 0 : rxnext + 1;
	gmacdev->RxNextDesc = last_rx_desc ? gmacdev->RxDesc : (rxdesc + 1);
#else
	gmacdev->RxNext     = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? 0 : rxnext + 1;
	gmacdev->RxNextDesc = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? gmacdev->RxDesc : (rxdesc + 1);
#endif

	TR("%02d %08x %08x %08x %08x %08x\n",rxnext,(u32)rxdesc,rxdesc->status,rxdesc->length,rxdesc->buffer1,rxdesc->data1);
	(gmacdev->BusyRxDesc)++; //One descriptor will be given to Hardware. So busy count incremented by one
	return rxnext;
}
...
/**
  ...
  * @param[out] virtual pointer for buffer1.
  * (@param[out] Dma-able buffer2 pointer ->removed).
  * (@param[out] pointer to hold length of buffer2 (Max is 2048) ->removed).
  * (@param[out] virtual pointer for buffer2 ->removed).
  * \return returns present rx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_get_rx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1)
{
#if 1//minimize function calls
	bool last_rx_desc;
#endif
	u32 rxnext       = gmacdev->RxBusy;	// index of descriptor the DMA just completed. May be useful when data 
						//is spread over multiple buffers/descriptors
...
#if 1//minimize function calls
	last_rx_desc = synopGMAC_is_last_rx_desc(gmacdev,rxdesc);
	gmacdev->RxBusy     = last_rx_desc ? 0 : rxnext + 1;
#else
	gmacdev->RxBusy     = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? 0 : rxnext + 1;
#endif
...
//use WAVE400_INLINE
#if 1//minimize function calls
	gmacdev->RxBusyDesc = last_rx_desc ? gmacdev->RxDesc : (rxdesc + 1);
	synopGMAC_rx_desc_init_ring(rxdesc, last_rx_desc);
#else
	gmacdev->RxBusyDesc = synopGMAC_is_last_rx_desc(gmacdev,rxdesc) ? gmacdev->RxDesc : (rxdesc + 1);
	synopGMAC_rx_desc_init_ring(rxdesc, synopGMAC_is_last_rx_desc(gmacdev,rxdesc));
#endif
...
/**
  ...
  * \return returns void.
  */
WAVE400_INLINE void synopGMAC_clear_interrupt(synopGMACdevice *gmacdev)
{
	u32 data;
...
/**
  ...
  * \return 0 upon success. Error code upon failure.
  */
WAVE400_INLINE u32 synopGMAC_get_interrupt_type(synopGMACdevice *gmacdev)
{
	u32 data;
	u32 interrupts = 0;
	data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus);
//	if (data & 0x000000ba)
//	  TR0("DMA status reg is  %08x\n",data);
	...
	if(data & DmaIntRxNormMask)	interrupts |= /*synopGMACDmaRxNormal*/0x00000040;
...
/**
  ...
  * \return returns void.
  */
WAVE400_INLINE void synopGMAC_enable_interrupt(synopGMACdevice *gmacdev, u32 interrupts)
{
...
/**
  ...
  */
WAVE400_INLINE void synopGMAC_disable_interrupt_all(synopGMACdevice *gmacdev)
{
...
#ifdef WAVE400_TEST_DMA
    printk("read (u32)gmacdev->RxDescDma as 0x%08x \n",synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaRxBaseAddr));
    printk("read DmaBusMode as 0x%08x",synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaBusMode));
    TR0("%s: read data = 0x%x, addr = 0x%x, ofset = 0x%x \n",__FUNCTION__,data,gmacdev->DmaBase, DmaControl);
#endif
  	data |= DmaRxStart; 
...
/**
  ...
  * \return returns void.
  */
/*WAVE400_INLINE*/ void synopGMAC_resume_dma_tx(synopGMACdevice * gmacdev)
{
	synopGMACWriteReg((u32 *)gmacdev->DmaBase, DmaTxPollDemand, 0);

#ifdef WAVE400_TEST_DMA
    TR0("DMA status is 0x%08x\n",synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus));
#endif
}

u32 synopGMAC_get_status_dma(synopGMACdevice * gmacdev)
{
    u32 val = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus);
//    TR0("DMA status is 0x%08x\n",val);
    return val;
}
...
/**
  ...
  * \return returns void.
  */
WAVE400_STATIC_INLINE void synopGMAC_take_desc_ownership(DmaDesc * desc)
{
	if(desc){
...
/**
  ...
  */
WAVE400_INLINE void synopGMAC_take_desc_ownership_rx(synopGMACdevice * gmacdev)
{
	s32 i;
	DmaDesc *desc;
	desc = gmacdev->RxDesc;
...
/**
  ...
  * \note Make sure to disable the transmission before calling this function, otherwise may result in racing situation.
  */
WAVE400_INLINE void synopGMAC_take_desc_ownership_tx(synopGMACdevice * gmacdev)
{
	s32 i;
	DmaDesc *desc;
	desc = gmacdev->TxDesc;
...
/**
  ...
  */
WAVE400_INLINE u32 synopGMAC_is_rx_checksum_error(synopGMACdevice *gmacdev, u32 status)
{
...
/**
  ...
  */
WAVE400_INLINE bool synopGMAC_is_tx_ipv4header_checksum_error(synopGMACdevice *gmacdev, u32 status)
{
...
/**
  ...
  * \return returns true if error in ipv4 header checksum, else returns false.
  */
WAVE400_INLINE bool synopGMAC_is_tx_payload_checksum_error(synopGMACdevice *gmacdev, u32 status)
{

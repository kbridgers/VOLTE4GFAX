...
#define IOCTL_MEM_DEBUG      SIOCDEVPRIVATE+9
...
typedef struct synopGMACDeviceStruct      
{
  ...
  /*DmaDesc*/void *TxDescFree;     /* start address of TX descriptors ring or chain, this is product of kmalloc  */
  /*DmaDesc*/void *RxDescFree;     /* start address of RX descriptors ring or chain, this is product of kmalloc  */
...
static int memdebug(char *ifname, int argc, char *argv[] )
{
  int socket_desc;
  int retval = 0, i;
  ...
  data.switch_case = strtol(argv[0],NULL,16); 		//convert the register address argument from string to long
  //printf("after strtol 0\n");
  data.data = strtol(argv[1],NULL,16); 		        //convert the register address argument from string to long
  //printf("after strtol 1\n");
  
  //printf("data.switch_case = 0x%x, data.data = 0x%x \n",data.switch_case, data.data);

  retval = ioctl( socket_desc, IOCTL_MEM_DEBUG, &ifr );
  if( retval < 0 )
   {
      printf("IOCTL Error");
      goto close_socket;
...
#ifdef DO_CPU_STAT
static int cpustats(char *ifname, int argc, char *argv[] )
{
  int socket_desc;
  int retval = 0, i;
  struct ifreq ifr;  //defined in if.h
  ...
  retval = ioctl( socket_desc, IOCTL_CPU_STATS, &ifr );
  if( retval < 0 )
   {
      printf("IOCTL Error");
      goto close_socket;
...
static int dmareset(char *ifname, int argc, char *argv[] )
{
...
//  ifr.ifr_data = (void *) &data;
  ...
  retval = ioctl( socket_desc, IOCTL_RESTART_DMA, &ifr );
  if( retval < 0 )
   {
      printf("IOCTL Error");
      goto close_socket;
  ...
  printf("The DMA is Successfully restarted \n");
...
#ifdef WAVE400_IOCTL_RESTART_DMA
    if( strcmp( argv[2], "dmareset") == 0)  retval = dmareset(argv[1],argc-3,argv+3);
#endif  
#ifdef DO_CPU_STAT
    if( strcmp( argv[2], "cpustats") == 0)  retval = cpustats(argv[1],argc-3,argv+3);
#endif  
    if( strcmp( argv[2], "memdebug") == 0)  retval = memdebug(argv[1],argc-3,argv+3);
  }

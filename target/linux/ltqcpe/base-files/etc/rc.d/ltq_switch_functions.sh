...
wan_port_mii0="4"
wan_port_xdsl="10 11"
wan_port_mii1_ar10f2="1"
wan_port_lte="7"
lan_port="0 1 2 4" 
lan_port_all="$lan_port $wan_port_mii1"
lan_port_ar10f2="2 3 4 5"
lan_port_mii0="0 1 2"
wlan_port="7 8 9"
wlan_port_all="7 8 9"
wlan_port_lte="8 9"
...
############## for AR10 F2 boards ##############
   if [ "$CONFIG_PACKAGE_KMOD_LTQCPE_AR10_F2_SUPPORT" = "1" ]; then
	wan_port_mii1="$wan_port_mii1_ar10f2"
	wan_reg_mii1="0x02"
	lan_port="$lan_port_ar10f2"
	lan_port_all="$lan_port $wan_port_mii1" 
   fi
################################################
...
switch_init() {
	...
	output=`switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_CFG_GET nPortId=0| grep -w nPortVId | awk '{ print $2 }'`
	...
	# egress tagging enabled on CPU port for that VLAN.
        for i in $wan_port; do
	   if [ $i = $wan_port_mii1 -o $i = $wan_port_mii0 -o $i = $wan_port_mii1_ar10f2 ]; then

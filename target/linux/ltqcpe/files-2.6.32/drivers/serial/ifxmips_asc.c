...
#define DBG_ENABLE_MASK_BUFFERING       (1 << 11)
#define DBG_ENABLE_MASK_ALL             DBG_ENABLE_MASK_BUFFERING
...
static int ifx_asc_verify_port(struct uart_port *, struct serial_struct *);
  ...
  void ifx_asc_poll_put_char(struct uart_port *, unsigned char);
  int ifx_asc_poll_get_char(struct uart_port *);
#endif
...
static int add_fdv_and_reload_value(unsigned int, unsigned int, unsigned int, unsigned int);
static int search_fdv_and_reload_value(unsigned int, unsigned int, unsigned int *, unsigned int *);
static void get_fdv_and_reload_value(unsigned int, unsigned int, unsigned int *, unsigned int *);
...
static void *proc_read_value_seq_start(struct seq_file *, loff_t *);
static void *proc_read_value_seq_next(struct seq_file *, void *, loff_t *);
static void proc_read_value_seq_stop(struct seq_file *, void *);
static int proc_read_value_seq_show(struct seq_file *, void *);
static int proc_read_value_seq_open(struct inode *, struct file *);
static ssize_t proc_file_write_value(struct file *, const char __user *, size_t, loff_t *);

static inline int ifx_asc_version(char *buf);

#ifdef CONFIG_CPU_FREQ
static int asc_cpufreq_notifier(struct notifier_block *nb, unsigned long val, void *data);

static struct notifier_block asc_cpufreq_notifier_block = {
	.notifier_call  = asc_cpufreq_notifier
};

extern struct list_head ltq_lxfreq_head_mod_list_g;

struct LTQ_LXFREQ_MODSTRUCT asc_lxfreq_mod_g = {
	.name							= "UART",
	.pmcuModule						= IFX_PMCU_MODULE_UART,
	.pmcuModuleNr					= 0,
	.powerFeatureStat				= IFX_PMCU_PWR_STATE_INVALID,
	.ltq_lxfreq_state_get			= ifx_uart_pwm_state_get,
	.ltq_lxfreq_pwr_feature_switch	= NULL,
};
...
static LIST_HEAD(g_fdv_reload_value_list);
...
#ifdef CONFIG_CONSOLE_POLL
    .poll_put_char  = ifx_asc_poll_put_char,
    .poll_get_char  = ifx_asc_poll_get_char,
#endif
};
...
static struct seq_operations g_proc_read_value_seq_ops = {
    .start  = proc_read_value_seq_start,
    .next   = proc_read_value_seq_next,
    .stop   = proc_read_value_seq_stop,
    .show   = proc_read_value_seq_show,
};
static struct file_operations g_proc_file_value_seq_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_read_value_seq_open,
    .read       = seq_read,
    .write      = proc_file_write_value,
    .llseek     = seq_lseek,
    .release    = seq_release,
};

#if defined(CONFIG_IFX_PMCU) || defined(CONFIG_CPU_FREQ)
...
#define F_BAUDRATE_CHG      0x02
    ...
    if ( f_chg ) {
        ...
        if ( (f_chg & F_BAUDRATE_CHG) ) {
            ...
            if ( baudrate ) {
                get_fdv_and_reload_value(ifx_get_fpi_hz(), baudrate, &fdv, &reload);
...
#ifdef CONFIG_CONSOLE_POLL
void ifx_asc_poll_put_char(struct uart_port *port, unsigned char c)
{
    if ( c == '\n' )
        ifx_asc_serial_out(g_asc_console_port,'\r');
    ifx_asc_serial_out(g_asc_console_port, c);
}

int ifx_asc_poll_get_char(struct uart_port *port)
{
    ifx_asc_port_priv_t *priv = &ifx_asc_port_priv[port->line];
    ifx_asc_reg_t *asc_reg = priv->base;
...
static int add_fdv_and_reload_value(unsigned int clk, unsigned int baudrate, unsigned int fdv, unsigned int reload)
{
    struct fdv_reload_value *new = kzalloc(sizeof(struct fdv_reload_value), GFP_KERNEL);    //  should use GFP_ATOMIC in case interrupt context?

    if ( !new )
        return -ENOMEM;

    new->clk      = clk;
    new->baudrate = baudrate;
    new->fdv      = fdv;
    new->reload   = reload;

    if ( list_empty(&g_fdv_reload_value_list) )
        list_add(&(new->list), &g_fdv_reload_value_list);
    else {
        struct list_head *pos, *n;
        struct fdv_reload_value *cur;

        list_for_each_safe(pos, n, &g_fdv_reload_value_list) {
            cur = list_entry(pos, struct fdv_reload_value, list);
            if ( clk < cur->clk || (clk == cur->clk && baudrate < cur->baudrate) ) {
                __list_add(&(new->list), pos->prev, pos);
                return 0;
            }
        }
        list_add_tail(&(new->list), &g_fdv_reload_value_list);
    }

    return 0;
}

static int search_fdv_and_reload_value(unsigned int clk, unsigned int baudrate, unsigned int *fdv, unsigned int *reload)
{
    struct list_head *pos, *n;
    struct fdv_reload_value *cur;
    ...
    list_for_each_safe(pos, n, &g_fdv_reload_value_list) {
        cur = list_entry(pos, struct fdv_reload_value, list);
        if ( cur->clk == clk && cur->baudrate == baudrate ) {
            if ( fdv != NULL )
                *fdv    = cur->fdv;
            if ( reload != NULL )
                *reload = cur->reload;
...
static void get_fdv_and_reload_value(unsigned int clk, unsigned int baudrate, unsigned int *fdv, unsigned int *reload)
{
    unsigned int __fdv = 0, __reload = 0;
    ...
    if ( baudrate >= (clk >> 4) ) {
        printk(KERN_ERR "%s current fpi clock %u can't provide baudrate %u!!!\n",
            __func__, clk, baudrate);
        return;
    }

    if ( search_fdv_and_reload_value(clk, baudrate, fdv, reload) == 0 )
        return;
    ...
    for ( bg = 1; bg <= 8192; bg++, fdv_over_bg_fpi += baudrate1 ) {
        fdv_over_bg = fdv_over_bg_fpi + clk / 2;
        do_div(fdv_over_bg, clk);
        if ( fdv_over_bg <= 512 ) {
            difference = fdv_over_bg * clk * 1000;
            ...
            if ( difference < min_difference ) {
                __fdv = (unsigned int)fdv_over_bg & 511;
                __reload = bg - 1;
    ...
    add_fdv_and_reload_value(clk, baudrate, __fdv, __reload);
    ...
    prom_printf("%s: clk = %u, baudrate = %u, fdv = %u, bg = %u, diff = %u.%u\n", __func__, clk, baudrate, __fdv, __reload, (u32)min_difference / 1000, (u32)min_difference % 1000);
#endif
}
...
static void ifx_asc_init_hardware(void)
{
    ...
    if ( !ifx_asc_hw_initialized ) {
        ...
        get_fdv_and_reload_value(ifx_get_fpi_hz(), CONFIG_IFX_ASC_DEFAULT_BAUDRATE, &fdv, &reload);
...
static inline void proc_file_create(void)
{
    ...
    res = create_proc_entry("value",
                            0,
                            g_asc_dir);
    if ( res )
        res->proc_fops = &g_proc_file_value_seq_fops;
...
static inline void proc_file_delete(void)
{
    remove_proc_entry("value", g_asc_dir);

    remove_proc_entry("dbg", g_asc_dir);
...
static int proc_read_dbg(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
    ...
    len += sprintf(buf + len, "buffering during clock change - %s\n", (g_dbg_enable & DBG_ENABLE_MASK_BUFFERING) ? "enabled" : "disabled");
...
static int proc_write_dbg(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    if ( strnicmp(p, "enable", 6) == 0 ) {
        p += 6;
        f_enable = 1;
    }
    else if ( strnicmp(p, "disable", 7) == 0 ) {
        p += 7;
        f_enable = -1;
    }
    else if ( strnicmp(p, "help", 4) == 0 || *p == '?' ) {
        printk("echo <enable/disable> [buf/all] > /proc/driver/ifx_asc/dbg\n");
    }
        ...
        if ( *p == 0 ) {
            ...
            do {
                for ( i = 0; i < ARRAY_SIZE(dbg_enable_mask_str); i++ )
                    if ( strnicmp(p, dbg_enable_mask_str[i], dbg_enable_mask_str_len[i]) == 0 ) {
                        if ( f_enable > 0 )
...
static void *proc_read_value_seq_start(struct seq_file *seq, loff_t *ppos)
{
    struct list_head *pos, *n;
    loff_t i;
...
static void *proc_read_value_seq_next(struct seq_file *seq, void *v, loff_t *ppos)
{
    struct list_head *pos = (struct list_head *)v;

    ++*ppos;
    return list_is_last(pos, &g_fdv_reload_value_list) ? NULL : pos->next;
}

static void proc_read_value_seq_stop(struct seq_file *seq, void *v)
{
}

static int proc_read_value_seq_show(struct seq_file *seq, void *v)
{
    struct fdv_reload_value *cur;

    cur = list_entry((struct list_head *)v, struct fdv_reload_value, list);
    if ( v == g_fdv_reload_value_list.next )
        seq_printf(seq, "clock\tbaud-rate\tfdv\treload\n");
    if ( cur->clk >= 1000000 )
        seq_printf(seq, "%4dM", (cur->clk + 500000) / 1000000);
    else if ( cur->clk >= 1000 )
        seq_printf(seq, "%4dK", (cur->clk + 500) / 1000);
    else
        seq_printf(seq, "%4d", cur->clk);
    seq_printf(seq, "\t%9d\t%d\t%6d\n", cur->baudrate, cur->fdv, cur->reload);

    return 0;
}

static int proc_read_value_seq_open(struct inode *inode, struct file *file)
{
    return seq_open(file, &g_proc_read_value_seq_ops);
}

static ssize_t proc_file_write_value(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    int len;
    char str[256];
    char *p;

    unsigned int clk, baudrate, fdv, reload;
    ...
    p++;
    for ( ; *p && *p <= ' '; p++ );
    baudrate = simple_strtoul(p, NULL, 0);
    if ( clk != 0 && baudrate != 0 ) {
        get_fdv_and_reload_value(clk, baudrate, &fdv, &reload);
        printk("fdv = %u, reload = %u\n", fdv, reload);
    }
    else {
        printk("echo <clock> <baud-rate> > /proc/driver/ifx_asc/value\n");
    }
...
static IFX_PMCU_RETURN_t  ifx_uart_pwm_prechange(IFX_PMCU_MODULE_t pmcuModule, IFX_PMCU_STATE_t newState, IFX_PMCU_STATE_t oldState)
{
    ...
    for ( i = 0; i < UART_NR; i++ ) {
        ...
        if ( g_clock_changing[i]++ == 0 ) {
            if ( (g_dbg_enable & DBG_ENABLE_MASK_BUFFERING) ) {
...
#ifdef CONFIG_CPU_FREQ
static void ifx_asc_pwm_cpufreq_init(void)
{
	if ( cpufreq_register_notifier(&asc_cpufreq_notifier_block,CPUFREQ_TRANSITION_NOTIFIER) ) {
					printk(KERN_ERR "Fail in registering ASC (UART) to CPUFREQ\n");
	}
	list_add_tail(&asc_lxfreq_mod_g.list, &ltq_lxfreq_head_mod_list_g);
}

static void ifx_asc_pwm_cpufreq_exit(void)
{
	if ( cpufreq_unregister_notifier(&asc_cpufreq_notifier_block,CPUFREQ_TRANSITION_NOTIFIER) ) {
					printk(KERN_ERR "Fail in unregistering ASC (UART) from CPUFREQ\n");
	}
	list_del(&asc_lxfreq_mod_g.list);
}


/* keep track of frequency transitions */
static int
asc_cpufreq_notifier(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if(new_State == IFX_PMCU_STATE_INVALID) {
		return NOTIFY_STOP_MASK | (IFX_PMCU_MODULE_UART<<4);
	}
	...
	if (val == CPUFREQ_PRECHANGE){
		ret = ifx_uart_pwm_prechange(IFX_PMCU_MODULE_UART, new_State, old_State);
        if (ret == IFX_PMCU_RETURN_DENIED) {
            return NOTIFY_STOP_MASK | (IFX_PMCU_MODULE_UART<<4);
        }
	} else if (val == CPUFREQ_POSTCHANGE){
		ret = ifx_uart_pwm_postchange(IFX_PMCU_MODULE_UART, new_State, old_State);
        if (ret == IFX_PMCU_RETURN_DENIED) {
            return NOTIFY_STOP_MASK | (IFX_PMCU_MODULE_UART<<4);
        }
	}else{
		return NOTIFY_OK | (IFX_PMCU_MODULE_UART<<4);
	}
	return NOTIFY_OK | (IFX_PMCU_MODULE_UART<<4);
}
#endif
...
void ifx_update_asc_clock_settings(void)
{
    ...
    for ( i = 0; i < NUM_ENTITY(ifx_asc_port); i++ ) {
        ...
        get_fdv_and_reload_value(ifx_get_fpi_hz(), ifx_asc_port_priv[i].baudrate, &fdv, &reload);
...
static void __init preload_fdv_and_reload_value(void)
{
#if defined(CONFIG_IFX_PMCU) && !defined(CONFIG_USE_EMULATOR)
  #if defined(CONFIG_AR9)
    get_fdv_and_reload_value(CLOCK_393M / 2, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_333M / 2, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_393M / 3, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_333M / 3, 115200, NULL, NULL);
  #elif defined(CONFIG_VR9)
    get_fdv_and_reload_value(CLOCK_300M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_250M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_200M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_196_608M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_167M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_125M, 115200, NULL, NULL);
  #elif defined(CONFIG_AR10)
    get_fdv_and_reload_value(CLOCK_300M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_250M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_150M, 115200, NULL, NULL);
    get_fdv_and_reload_value(CLOCK_125M, 115200, NULL, NULL);
  #endif
#endif
}
...
#ifdef CONFIG_CPU_FREQ
    ifx_asc_pwm_cpufreq_init();
#endif

    proc_file_create();

    preload_fdv_and_reload_value();

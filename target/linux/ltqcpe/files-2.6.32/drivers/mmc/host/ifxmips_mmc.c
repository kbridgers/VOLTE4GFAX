...
#define IFX_DRV_MODULE_VERSION          "1.0.9"
...
#define CONFIG_IFX_MMC_BLOCK_DEV
...
static void ifx_sdio_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if (ios->clock) {
        if (ios->clock >= host->mclk) {
            ...
            clk = MMC_READ_REG32(SDIO_CLC);
            clk = SDIO_CLC_RMC_GET(clk);
    //printk("%s[%d] clk = %d\n",__func__,__LINE__,clk);
            host->cclk = 2*(host->mclk) / clk;
            ifx_priv->current_speed = host->cclk;
        }
    clk = MMC_READ_REG32(MCI_CLK);
    clk |= (MCI_CLK_ENABLE);
    MMC_WRITE_REG32(clk, MCI_CLK);
    }

    //printk("%s[%d] ios->clock = %d, host->mclk = %d, host->cclk = %d \n",__func__,__LINE__,ios->clock, host->mclk,host->cclk);
    //printk("%s[%d] bus_width = %d\n",__func__,__LINE__,ios->bus_width);
...
static int ifx_sdio_probe(struct device *dev)
{
    ...
    int ret = 0;
#ifdef CONFIG_IFX_MMC_BLOCK_DEV
    mmc = mmc_alloc_host(sizeof(struct ifx_sdio_host), dev);
    if (!mmc) {
...
static int ifx_sdio_probe(struct device *dev)
{
    ...
    ifx_mmc = mmc_alloc_host(sizeof(struct ifx_sdio_host), dev);
    if (!ifx_mmc) {
                printk(KERN_ERR "%s: mmc alloc host failed!!!\n", __func__);
                retval = -ENOMEM;
                return retval;
    ...
    ifx_mmc->max_req_size = 65535;
    ifx_mmc->max_seg_size = ifx_mmc->max_req_size;
    /*
     * Block size can be up to 2048 bytes, but must be a power of two.
     */
    ifx_mmc->max_blk_size = 2048;
    /*
     * No limit on the number of blocks transferred.
     */
    ifx_mmc->max_blk_count = ifx_mmc->max_req_size;
...
/* Print the driver version info */                      
    mmc_host_drv_ver(ver_str);
        ...
        dev_set_drvdata(dev, ifx_mmc);
                        
#ifdef CONFIG_IFX_MMC_BLOCK_DEV
        mmc_add_host(ifx_mmc);
#endif

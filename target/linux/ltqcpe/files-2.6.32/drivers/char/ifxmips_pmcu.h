...
/**
   The function may used to measure runtimes in the range of 1us-sec's. 
   Base counter is the MIPS counter, also called CP0_Count which run at CPU/2. 
   The diff function is always used in combination of the ltq_count0_read() 
   function.
   
   \param[in]   count_start  start value
   \param[in]   count_end  end value
   \param[in]   ident  identifier string for printk (if NULL no printk)
   \param[in]   index  index to fill an timing value array 
    					used for pre,state,post processing timimgs accessable
    					via /proc/drivers/ifx_cpufreq/cpufreq_latency

   \return Returns the count0 diff value
	\code 

...
int32_t ifx_ppa_get_multifield_max_flow(uint32_t flag)   //hook
{
    ...
    res = ifx_ppa_drv_get_multifield_max_entry( &count, count.flags);
...
{ //note, in vlan information auto-learnign, here set all vlan related mask to zero
    ...
    if( p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname[0] != 0 )  //for auto and manul 
    {
        ...
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: ppa_netif_update fail for rx interface %s\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            return IFX_FAILURE;
        }

        if ( ppa_netif_lookup(p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname, &rx_ifinfo) != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: Rx interface %s ppa_netif_lookup fail\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            return IFX_FAILURE;
        }        
        if ( !(rx_ifinfo->flags & NETIF_PHYS_PORT_GOT) )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: Rx interface %s no physical port id\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }
    ...
    if( p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname[0] != 0  ) //for auto and manul 
    {
        ...
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: Tx interface %s ppa_netif_lookup fail\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }        
        if ( !(tx_ifinfo->flags & NETIF_PHYS_PORT_GOT) )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: Tx interface %s no physical port id\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }
    }
    ...
    if ( (tx_ifinfo->flags & NETIF_PHY_ATM) )  //for auto and manual
    {
        ...
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: mapped queue id from %d to %d\n", old_qid, p_multifield_info->cfg0.queue_id);
    }
...
int32_t ifx_ppa_del_multifield_flow( PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag )   //hook
{
    ...
    if( (res = ifx_ppa_auto_learn_multifield_flow(p_multifield_info ) ) != IFX_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_del_multifield_flow learning fail\n");
        return res;
    }  
    ...
    res = ifx_ppa_drv_del_multifield_entry( &flow, 0);
     if( res == IFX_SUCCESS )
    {     
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_del_multifield_flow: suceed to add below flow to index %d:\n", flow.entry);
    }
    else
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_del_multifield_flow: fail to add below flow---- \n");
    }
...
int32_t ifx_ppa_quick_del_multifield_flow( int32_t index, uint32_t flag)   //hook
{
    ...
    if( index == - 1)  // delete all mfe flow
    {
        ...
        for( i = 0; i < num; i++ )
        {
            flow.entry = i;
            flow.flag = flag;
            res = ifx_ppa_drv_del_multifield_entry_via_index(&flow, flag);
        }
    }
    else 
    {        
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    res = ifx_ppa_multifield_control( cmd_info->mf_ctrl_info.enable_multifield, cmd_info->mf_ctrl_info.flag);
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    res = ifx_ppa_get_multifield_flow( cmd_info->mf_flow_info.index, &cmd_info->mf_flow_info.flow, cmd_info->mf_flow_info.flag );
    ppa_debug(DBG_ENABLE_MASK_MFE, "ppa_ioctl_get_multifield_flow with res: %x\n", res);
    
    if(  res == IFX_FAILURE )
    {
        cmd_info->mf_flow_info.flag = IFX_FAILURE;
        res = IFX_SUCCESS; //note: here return IFX_SUCESS with cmd_info->mf_flow_info.flag setting to IFX_FAILURE in order to avoid ioctl failure for not used mf flow 
    }
    else if( res == IFX_SUCCESS )
    {
        cmd_info->mf_flow_info.flag = IFX_SUCCESS;
        res = IFX_SUCCESS;
    }
    else
    {

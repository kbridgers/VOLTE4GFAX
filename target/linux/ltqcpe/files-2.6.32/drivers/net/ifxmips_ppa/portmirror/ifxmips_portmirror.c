...
static int proc_read_port_mirror(char *, char **, off_t, int, int *, void *);
static int proc_write_port_mirror(struct file *, const char *, unsigned long, void *);
...
static INLINE void proc_file_create(void)
{
    ...
    if ( res )
    {
        res->read_proc  = proc_read_port_mirror;
        res->write_proc = proc_write_port_mirror;
    }
}
...
static int proc_read_port_mirror(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int proc_write_port_mirror(struct file *file, const char *buf, unsigned long count, void *data)
{
    char str[2048];
    char *p;
    int len, rlen;

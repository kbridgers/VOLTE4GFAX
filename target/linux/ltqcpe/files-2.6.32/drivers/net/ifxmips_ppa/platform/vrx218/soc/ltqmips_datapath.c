...
int eth_xmit(struct sk_buff *, unsigned int, int, int, int);
...
int eth_xmit(struct sk_buff *skb, unsigned int port, int ch, int spid, int class)
{
    struct sw_eg_pkt_header pkth = {0};
    int len;
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    case 7:     //  ATM EoA or PTM
        if ( g_smartphy_push_fn ){
            if(g_smartphy_push_fn(skb, header, off) == 0 ){
                return 0;
            }
            break;
        }
...
static INLINE int alloc_dma(void)
{
    ...
    g_dma_device->rx_chan[3]->desc_base = (int)DMA_RX_CH3_DESC_BASE;        //  with VRX318 acceleration

...
static irqreturn_t mii1_gphy_VSC_int_handler (int irq, void *dev_id)
{
    ...
    status=ar9_switch_read_mdio(PHY1_ADDR,0x1a); 
    /*interrupt status bit */
    if(status & 0x8000) { 
        /*cause the interrupt due to link status, speed & FDX */
        if( (status & 0x7000)) {
            linkReg=ar9_switch_read_mdio(PHY1_ADDR,0x01);
            /*Link on PHY is down */
            ...
            if((linkReg&0x04)==0) {
                ...
                speedReg=ar9_switch_read_mdio(PHY1_ADDR,0x1c);
    ...
    /*cause the interrupt due to link status, speed & FDX */
        if( (status & 0x0007)) {
            ...
            if((linkReg&0x04)==0) {
                ...
                speedReg=ar9_switch_read_mdio(PHY1_ADDR,0x18);
                 reg_rgmii = IFX_REG_R32(IFX_AR9_ETH_RGMII_CTL);
                if(speedReg & 0x08) {
                    reg_rgmii |= RGMII_CTL_DUP_SET(1,1);
                    phy_status->duplex = DUPLEX_FULL;
...
**  Strict Priority scheduling is used for all ports and Rate limit is disabled*/
	...
	id = ar9_switch_read_mdio(PHY1_ADDR, 0x3);
	switch ((id >> 4) & 0xf)
	{
		case 2:
    			ar9_switch_write_mdio(PHY1_ADDR, 0x19, 0xf000);
				g_id = GPHY_VSC;
				printk("\n GPHY_VSC ");
				break;
		case 0:
		case 13:
				ar9_switch_write_mdio(PHY1_ADDR, 0x19, 0x0007);
    ...
    /* Low Level (GPHY_VSC uses Falling Edge <0x02> which could be covered by Low Level <0x06> trigger) */
    reg = IFX_REG_R32(IFX_ICU_EIU_EXIN_C );
    reg |= (0x6 << (MII1_GPHY_INT_EXT_INT_NO * 4) );

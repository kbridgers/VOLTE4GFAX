...
static long ifx_eeprom_unlocked_ioctl(struct file *filp, unsigned int cmd,
				       unsigned long arg)
{
    long ret;

    ret = ifx_eeprom_ioctl(filp->f_path.dentry->d_inode, filp, cmd, arg);
    return ret;
}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
    .unlocked_ioctl = ifx_eeprom_unlocked_ioctl,
#else

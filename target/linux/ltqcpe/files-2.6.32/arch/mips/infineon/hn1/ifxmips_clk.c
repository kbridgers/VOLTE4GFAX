...
** 11 Mar 2011  Yinglei Huang   Adapted for HN1
*******************************************************************************/
/*! 
  \file ifxmips_clk.c
  \ingroup IFX_CGU
...
#include <asm/ifx/hn1/emulation.h>
#endif
...
#define CGU_CLKFSR_ETHE                 ((*IFX_CGU_CLKFSR >> 24) & 3)
 ...
 */
static u32 gp_strap = 0;
static inline u32 get_input_clock(void)
{
    // gpio13 is the boot strap ping,its reset value defines the reference clock
    // 0 = 37.5MHz, 0 = 25 MHz. The ping's value should be read earlier after reset
    // to prevent it been re-programmed by other module later.
    u32 freq;

    switch ((gp_strap >> 13) & 1)
    {
    case 0:
        freq = 37500000;
        break;
...
/*
 *  Description:
 *    get PLL0 clk "f_pll0 = f_ref * 2 (N+1)/(M+1)"
  *  Input:
 *    none
 *  Output:
...
/*
 *  Description:
 *    get PLL1 clk "f_pll1 ~= f_ref * (N+1)/(M+1)" with fractional adjustment
 *  Input:
 *    none
 *  Output:
 ...
 *    On HN1 fpi has the same speed as DDR.
 *  Input:
 ...
 *    register CGU_CLKFSR[ETH] provides this frequency
 *  Input:
...
static void update_cgu(u32 cgu_sys, u32 mem_reconfig, u32 dll_start_point)
{
    *IFX_CGU_SYS = cgu_sys;
    __sync();
...
static inline void update_sysclock(u32 cgu_sys)
{
    ...
    {
        case (2 << 4): i = CLOCK_400M; break; //cgu_get_pll1_fosc();
        case (5 << 4): i = CLOCK_200M; break;
        default:       i = CLOCK_200M;
    }
...
u32 cgu_set_clock(u32 cpu_clk, u32 ddr_clk, u32 fpi_clk)
{
    ...
    u32 cgu_sys_templ[] = { // DDR_CLK 400MHz is not supportted
	400, 200, 0x22,
	200, 200, 0x50,
	200, 100, 0x52,
    };
...
//        printk("TCSTATUS_A = 0x%p\n", (read_tc_c0_tcstatus() & TCSTATUS_A));
    ...
    ifx_fill_lock_icache(update_cgu, (u32)dummy_func - (u32)update_cgu);
 ...
 #endif
    ehb();
    update_cgu(cgu_sys, mem_reconfig, dll_start_point);
 #if defined(ENABLE_SET_CLOCK_PROFILING) && ENABLE_SET_CLOCK_PROFILING
    count_end = read_c0_count();
 #endif

    ifx_unlock_icache(update_cgu, (u32)dummy_func - (u32)update_cgu);
...
unsigned int ifx_get_cpu_hz(void)
{
    ...
    switch ( *IFX_CGU_SYS & (0x0f << 4) )
    {
        case (2 << 4): return CLOCK_400M; //cgu_get_pll1_fosc();
        case (5 << 4): return CLOCK_200M;
        default:       return CLOCK_200M;
    }
#endif
}

...
/*! \enum  IFX_USIF_SPI_DLX_t  
    \brief Set communication duplex mode of USIF SPI connection
    
    The duplex mode is used to notify USIF SPI bus driver by USIF SPI device driver about what kind of 
    communication mode should be used. Which duplex mode will be used depends on the USIF SPI device
    driver instead of USIF SPI bus driver.
 */
typedef enum{
    IFX_USIF_SPI_HALF_DUPLEX = 0, /*!< Half Duplex. Interface is used in half duplex when
                                  calling \ref ifx_usif_spiTxRx or \ref ifx_usif_spiAsyncTxRx
                                  The TX path is servered before the RX path. */
    IFX_USIF_SPI_FULL_DUPLEX,     /*!< Full Duplex. Interface is used in full duplex when
                                  calling \ref ifx_usif_spiTxRx or \ref ifx_usif_spiAsyncTxRx.
                                  The TX-and RX- path is servered simultaneously. */
}IFX_USIF_SPI_DLX_t;
...
typedef struct{
    ...
    IFX_USIF_SPI_DLX_t   duplex_mode; /*!< Duplex Mode Selector. Connection used the USIF SPI interface either in half- or full- duplex mode. */
